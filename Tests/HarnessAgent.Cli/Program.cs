...
{ using var t2 = new MockMcpTransport(); using var c = new McpClient(t2); await c.InitializeAsync(); await c.ListToolsAsync(); }

static async Task TestMcpServer()
{ var s = new McpServer(); s.RegisterTool("x", "", JsonSerializer.SerializeToElement(new {}), (_, __) => Task.FromResult("ok")); await s.HandleRequestAsync("{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"initialize\"}"); }

static void TestScreenCapture()
{ var m = ScreenCapture.GetMonitors(); if (m.Count == 0) { Console.WriteLine("  headless skip"); return; } Console.WriteLine($"  {m.Count} monitor(s)"); }

static void TestUIAutomation()
{ var a = WindowAutomation.GetActiveWindow(); if (a == IntPtr.Zero) { Console.WriteLine("  headless skip"); return; } Console.WriteLine($"  active: {WindowAutomation.GetWindowTitle(a)}"); }

static void TestSelfHealRestart()
{ try { var shr = new SelfHealRestart(); var lay = shr.Discover(); Console.WriteLine($"  window: {lay.WindowTitle} {lay.WindowBounds.Width}x{lay.WindowBounds.Height}"); } catch (InvalidOperationException) { Console.WriteLine("  Codex not running - skip"); } }
