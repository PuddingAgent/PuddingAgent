...
我们需要支持指令，比如/model、/yolo等指令，支持输入/的时候自动补全和选择，比如/m的时候。
...
FIX：需要支持多模型和并行的状态和查看、管理。
FIX：之后在右边显示todo-list，也就是任务的完成进度，灵感来自于https://github.com/code-yeongyu/oh-my-opencode

FIX：我们需要支持多模型和多agent和并行任务执行。


FIX：我们需要一个调度机制，来协调多个agent不会出现编辑冲突或者打架，也就是需要一个leader，Leader可以是一个LLM也可以是软件实现的调度网关。
