...
七个 Smart 工作流工具的子任务上限统一为 1800 秒。该值不是从工具调用时重新获得的
独立预算。`ExecutionRunCoordinator` 在父 Turn 启动时冻结唯一
...
`SmartWorkflowToolBase` 在父截止时间前固定预留 120 秒，供主 Agent 消化子代理报告、
生成最终回复并提交 terminal。有效 Smart 预算为：

```text
min(显式 timeout_seconds, 1800s, parentDeadlineUtc - now - 120s)
```

剩余预算不足时，Smart 工具必须在创建子代理前返回
...
| 组件 | 负责 | 不负责 |
|---|---|---|
| `SmartWorkflowToolBase` | 从配置所有者选择角色模型、以父 deadline 收紧 1800 秒子任务上限并预留父级收尾时间、声明有界委派元数据；校验 Smart 子代理返回的 canonical 详细工作报告并保留失败结果信封 | 创建 run、持久状态、任意递归或发明第二种结果格式 |
| `SubAgentTool` | 参数/权限/路由快照映射 | 重新解析 Provider、写 UI 状态 |
| `SubAgentInvocationService` | 生成 invocation/batch 身份、映射请求 | 执行或归档 |
| `SubAgentManager` | 归一化子代理绝对 deadline、把并发等待计入预算、创建 run 并派发 Runtime | 放宽父 deadline、解析会话字符串、维护第二套终态 |
...
18. 七个 Smart 工具未显式传入预算时子任务上限均为 1800 秒；若父 Turn 剩余预算
    更短，run archive 的 deadline 必须等于收紧后的父 deadline，绝不能晚于父 deadline。
...
21. 父 Turn 为 2400 秒且 Smart 子任务上限为 1800 秒时，Smart 最迟在父 deadline
    前 120 秒结束；父 Agent 有确定的收尾窗口，不会在收到子代理结果前被父 timeout
    抢先取消。
