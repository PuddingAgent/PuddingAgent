...
`SubAgentActivityDock` 和 `SubAgentAnchor` 只消费该投影，不再调用
...
子代理状态按用途拆成三个展示层，禁止把完整实时状态重新塞回消息虚拟列表：
...
2. 详情检查器显示 reducer 中有界保存的 canonical 活动时间线，包含阶段、轮次、
   ...
   文本预览必须经过 KeyVault 脱敏并限制长度；不得展示隐藏原始思维链、完整 Prompt、
   密钥或完整工具输出。检查器必须显示可复制的 `subSessionId` 与 `runId`。
3. `SubAgentAnchor` 是消息流中的稳定因果锚点，按 `batchId / invocationId /
   parentToolCallId` 聚合，只显示启动数量和终态计数。锚点高度不得随工具输出增长。
...
| `SubAgentActivityDock` | 固定运行态、详情检查器与本地退场策略 | 轮询后端、修正状态、进入虚拟消息流 |
| `SubAgentAnchor` | 会话因果锚点与批次终态摘要 | 展示完整实时细节、改变运行状态 |
...
13. 消息流只渲染稳定子代理锚点，实时工具输出不会改变虚拟项高度。
