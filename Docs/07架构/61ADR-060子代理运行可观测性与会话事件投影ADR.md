...
七个 Smart 工作流工具的单次子任务上限统一为 3600 秒，`smart_explore` 保持较小的
1800 秒只读探索上限。该值不是从工具调用时重新获得的独立预算。
`ExecutionRunCoordinator` 在父 Turn 启动时冻结唯一
`ExecutionDeadlineUtc`，随后沿
`TurnExecutionContext → RuntimeDispatchRequest → ToolInvocationRequest →
ToolExecutionContext → SubAgentInvocationRequest → SubAgentSpawnRequest`
逐层透传，任何下游只能缩短，禁止用 `UtcNow + timeout` 放宽。

所有同步子代理都由 `SubAgentManager` 在父截止时间前预留配置项
`parentFinalizationReserveSeconds`（默认 120 秒），供主 Agent 消化子代理报告、生成最终
回复并提交 terminal；异步子代理仍服从父 deadline，但不占用同步收尾预留。
`SmartWorkflowToolBase` 读取同一配置，并额外应用角色上限（默认 3600 秒，
`smart_plan=3600 秒/48 轮`，`smart_explore=1800 秒/32 轮`）。有效 Smart 预算为：

```text
min(显式 timeout_seconds, roleCap, parentDeadlineUtc - now - 120s)
```

剩余预算不足时，Smart 工具或同步 `spawn_sub_agent` 必须在创建子代理前返回
`insufficient_execution_budget`。`SubAgentManager` 再以父 deadline 作为硬上界，
...
### 3.10 父 Turn 的硬上限与滑动无进展看门狗（2026-07-22 演进）

父 Turn 不再把 2400 秒固定时长当成正常终止条件。默认 Agent 快照与平台 Guardrail
统一使用 86400 秒（24 小时）作为最终安全上限；它只负责清理失控且无法被其他机制
终结的 Run，不代表预期任务时长。

正常停滞检测由进程内 `IExecutionProgressRegistry` 和
`ExecutionWatchdogPolicy` 完成：

- 默认 `noProgressTimeoutSeconds=3600`，每 5 秒检查一次；
- LLM 新文本、推理片段、工具调用参数、LLM 完成、工具完成，以及同一 Conversation
  下子代理产生的这些事实，属于有效进展并滑动续期；
- Worker lease 续租、SSE keepalive、连接仍存活和空 usage 帧只属于 liveness，不续期；
- 相同 Run、相同阶段、相同指纹的重复片段不算新的有效进展，避免完全相同的复读输出
  无限续期；
- 超过无进展窗口写入 `execution_stalled`，达到 24 小时硬上限仍写入
  `execution_timeout`，显式用户取消仍写入 `cancelled`。

LLM 单次流另有更短的操作级看门狗：默认首块等待 300 秒，收到首块后的相邻流块空闲
上限为 120 秒。Provider 的 `streamTimeoutSeconds` 只可进一步收紧这两个空闲窗口，不再
...
| `ISubAgentRunStore` | 运行审计、终态仲裁、从持久事件合并真实终态统计、可靠投影 | Agent 编排 |
...
18. Smart 工具未显式传入预算时使用角色上限：默认 3600 秒，`smart_plan=3600 秒/48 轮`，
    `smart_explore=1800 秒/32 轮`；若父 Turn 剩余预算更短，run archive 的 deadline 必须
    等于收紧后的父 deadline，绝不能晚于父 deadline。
...
21. 父 Turn 使用 86400 秒硬上限和 3600 秒无进展窗口；子代理持续产生有效进展时会
    续期父看门狗，但每次同步 Smart 调用仍必须在父 deadline 前 120 秒结束，给父 Agent
    留出确定的收尾窗口。
22. Smart 报告合同校验失败时，工具错误包含 `subAgentId/runId/validationError`，
    `Output` 保留完整 `spawn_sub_agent` 结果信封和 `rawOutput`，不得只留截断预览。
23. 同步 `spawn_sub_agent` 在父 Turn 只剩收尾预留时返回
    `insufficient_execution_budget`，不得创建一个耗尽父 deadline 的重试 run。
24. 子代理在超时/取消前已有 round/tool 事件时，terminal 与 DB 索引必须从 archive 合并
    真实轮次、工具数、耗时及工具失败统计，禁止归零。
25. 仅 lease/SSE/空帧持续活跃 3600 秒时，父 Run 以 `execution_stalled` 终结；新的 LLM、
