...
### 4.2 2026-07-24 消息阶段视觉与动效边界

等待、推理、工具活动、流式正文和完成正文可以共享同一套视觉语法，但动效状态只能是
canonical 消息/运行投影的纯呈现结果，不能成为新的会话事实来源。具体约束如下：

- 入场动画仅用于活动消息或创建后短窗口内的消息；虚拟列表滚动、历史分页和刷新后的行重挂载
  不得让历史消息重新播放入场动画；
- 等待粒子和回答完成粒子不得影响消息高度、滚动锚点或 viewport 测量，且必须遵循
  `prefers-reduced-motion`；
- 活动扫光、边缘呼吸和停滞告警只改变绘制层，不得新增轮询、计时所有权或修改
  turn/message/session 身份；
- 多阶段面板统一宽度、圆角和状态轨道，但正文可读性优先，光晕和粒子不得持续覆盖内容底色。
