...
> 2026-09-15 后续裁决：[ADR-092](../07架构/106ADR-092目标驱动执行与分层验证闭环ADR.md) 与[分层验证施工方案](../Features/Goal目标驱动执行与分层验证闭环设计-2026-09-15.md)在冲突处优先：增加真正检查与步骤/整体两级校验、durable wait 恢复、配置授权的长程预算和可选重启恢复；不把 TaskCompleted、单轮结束或 sleep 当作 Goal 完成。下文保留历史设计，固定 256 等被替代条款不再作为新实施约束。
