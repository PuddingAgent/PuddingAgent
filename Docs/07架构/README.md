...
后台异步索引维护已纳入 ADR-089 与详细设计 §10：FileSystemWatcher、有界队列、持久待办、增量更新/失效、完整扫描校准与安全索引清理；由既有 U3 承接，U5 增加异常恢复与删除一致性门禁，尚未实施。

已登记总任务 `b74e561e5299479e9afdd5123f26490d` 与六张 U0–U5 阶段卡；P1/Backlog、未派发，映射与验收依赖见详细设计 §9。

[ADR-089](103ADR-089Agent统一检索与渐进展开工具链ADR.md)（Proposed）与[详细设计](../Features/Agent统一检索与渐进展开工具链设计-2026-09-13.md)：一个搜索入口聚合 Everything、代码索引与现有 Lucene/托管内容检索，一个展开入口统一读取/结构/关系；共享范围、证据引用、版本与预算。仅借鉴 tgrep 思路，不引入其程序/服务；本轮仅设计，未实施或验收。
...
## 2026-09-12 新阶段ADR（Proposed）

- [ADR-084 稳定请求前缀与缓存99](98ADR-084稳定请求前缀与缓存99验收ADR.md)
- [ADR-085 Memory主导的长程任务连续性](99ADR-085Memory主导的长程任务连续性ADR.md)
- [ADR-086 600轮长程执行与内核收敛](100ADR-086长程执行预算与运行内核收敛ADR.md)
