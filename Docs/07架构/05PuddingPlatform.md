...
5. **监测优先，干预可选**
	...
	- 对安全风险应提供一键 stop / recover 的紧急治理入口
...
- Agent 当前任务
- role
- 当前状态：`idle / busy / blocked / sleeping / hibernated / frozen`
- 当前工作负载
- 最近一次关键动作
...
- 当前是否处于 `Workspace Frozen`
- 已冻结 Agent 数
- 已停止容器数
- 最近一次紧急 stop 发起者与时间

## Workspace 紧急 stop / recover 按钮

Workspace 驾驶舱应提供显式的紧急治理按钮：
...
- `Stop Workspace` 是高优先级紧急按钮
- 用户点击后，应快速冻结当前 Workspace 的全部 Agent
- 并向 Controller / Runtime 下发全局冻结请求，停止对应 Docker 容器或 sandbox
- `Recover Workspace` 用于在确认风险解除后恢复 Workspace 的 Agent 与容器

建议原则：

- `Stop Workspace` 默认应放在 Workspace 驾驶舱顶部的明显区域
- 需要二次确认，但确认流程必须足够快，避免“紧急按钮点出了审批流程迷宫”
- 执行结果应立即反馈：冻结了多少 Agent、停止了多少容器、是否有失败项
