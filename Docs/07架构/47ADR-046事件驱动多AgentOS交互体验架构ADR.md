...
当前 UI 已有大量增量功能：Chat、Composer、语音按钮、DevPanel、子 Agent 卡片、记忆/缓存/Token 指示器、命令面板、主题 token。但这些能力仍像多个局部增强拼在一起，缺少统一的信息架构、多模态权限模型和交互状态模型。曾用于像素化状态表达的 Workspace Studio 已在 2026-08-09 因前端体积与维护成本被移除，其精灵素材仅作为未来原生客户端资源保留。
