...
用户最新要求改由 Codex 直接推进修复。A1 已交付 Runtime 87/87、Platform 32/32；本批还修复搜索挂起、回复投影毒项和恢复前取消，合计204项定向测试通过，并完成新 Core 两次真实工具调用与 canonical 终态验收。详见[接管修复与部署证据](默认助手停滞修复与预算接管-2026-09-14.md)。以下设计合同保留供后续施工使用；不要再把本文件作为未完成的 A1 指令重复派发给 PuddingAgent。
...
## 2. A2 / P0：压缩口径纠偏（压力口径已部署，无收益抑制待实施）

2026-09-15后续决策优先于下方原任务书：572c394直接删除固定raw cap，不再实现“候选原文超cap也触发”；两条默认压力入口统一80%，分离Provider实报/本地估算，排除压缩代际以前的usage。141项回归通过、Core PID24908真实只读工具成功，见[修复报告](百万上下文频繁压缩修复-2026-09-15.md)。后续聚焦第2项无收益候选抑制和真实工作负载归一化验收；第1项候选统计仅用于收益评估，不得恢复固定128K自动压缩。以下保留当时规划以便追溯。
