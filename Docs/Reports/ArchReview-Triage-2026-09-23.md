...
| 条目 | ADR | 状态 |
|---|---|---|
| 第 1 条 | `Docs/07架构/107ADR-093子代理运行归档权威链水位与降级台账ADR.md` | Proposed |
| 第 4 条 | `Docs/07架构/108ADR-094Host启动配置单一来源与生效来源审计ADR.md` | Proposed |

两份 ADR 均含：对评审表述的**逐行核对与更正**、决策、备选方案取舍、分阶段发布与可判定验收、
以及「实施前必须由实施者自行回填」的待核项（不得凭 ADR 转述）。
