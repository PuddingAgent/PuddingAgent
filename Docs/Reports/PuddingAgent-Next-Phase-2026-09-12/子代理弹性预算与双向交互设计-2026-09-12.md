...
本文件补充并修订[长程自治方案](01-设计方案.md)和[ADR-086](adrs/100ADR-086长程执行预算与运行内核收敛ADR.md)；配套[ADR-087](adrs/101ADR-087子代理托管运行与持久问答ADR.md)。**600 只是可选预算示例，不是统一默认要求、下限或上限。** 短轮次、600、1200以及其他合法正整数均应支持。
...
> **实施状态（2026-09-12，N00，commit f096bc5）**：已完成——Manager managed Math.Min(40/120)、默认32强制选择、Normalize强抬600、SubAgentInvocationContracts 三常数（DefaultWorkUnitMaxRounds/MaxWorkUnitMaxRounds/DefaultWorkUnitMaxToolCallsTotal）删除。待完成（已登记的重开纠正片）——TaskExecutionPlanCompiler 阶段小预算、AgentExecutionService.Buffered 对 >600 的 Math.Min 压制、AgentExecutionGuardrails 上界语义、SubAgentInvocationContracts.MaxRounds 的 XML 注释「Parent agents cannot override it」纠正。
...
具体任务ID、写入前后版本和下发回执见同日[看板修订记录](04-子代理任务修订.md)。
