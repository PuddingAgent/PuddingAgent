...
配套：[ADR-084](adrs/98ADR-084稳定请求前缀与缓存99验收ADR.md)、[ADR-085](adrs/99ADR-085Memory主导的长程任务连续性ADR.md)、[ADR-086](adrs/100ADR-086长程执行预算与运行内核收敛ADR.md)。[任务路线、看板回执和证据包](README.md)。
