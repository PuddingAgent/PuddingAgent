...
## 2026-06-20: Agent 上下文陷阱——信任描述胜过信任验证

### 经验：Agent 对工具可用性的判断应优先通过实际调用验证，而非依赖静态分析或上下文标注
- **场景**：Agent 在决策时，上下文中的工具列表可能带有过时或不准确的描述性标注（如"需显式授权"），导致 Agent 误判某工具不可用。
- **案例**：
  - dev Agent 以为 `file_write` 不可用（标注"需显式授权"，实际直接可用）
  - dev Agent 以为 `sleep` 不可用（之前误判惯性）
  - 默认助手判断 `sleep` 未注册为 `IPuddingTool`（静态分析，实际注册为 `ITool + IAgentSkill`）
  - dev Agent 从自然语言内容推断自己是 Workspace Service Agent（实际是代码助手）
- **系统防御**：
  - `constraints` 中 "Use metadata to identify sender" — 防身份误判 ✅
  - `Isolation = EventIsolationMode.Isolated` — 心跳不污染主会话 ✅
  - 工具实际可用性验证 — **缺乏**，需在 Agent System Prompt 增加原则 ❌
- **改进方向**：在 Agent L0/SOUL 层增加系统原则：
  > 当对某个工具或能力的可用性有疑问时，优先通过实际调用测试来验证，而不是仅依赖静态分析或上下文中的描述性标注。