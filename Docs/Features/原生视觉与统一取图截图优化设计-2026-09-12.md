...
> 决策：[ADR-088](../07架构/102ADR-088原生视觉取图与截图统一链路ADR.md)。延续 ADR-077 已交付的 Artifact、typed parts 和 Files API；本轮不修改产品源码、运行配置或数据。
...
`8 张` 可保留为第一阶段产品默认值，但必须在唯一策略源定义并对主/子代理一致生效，可在受支持范围内配置；它不是厂商上限。同样，图像质量/裁剪由统一策略定义，不在前端、Reader、Planner 各压一次。Provider 边界按 endpoint/model/版本维护；预算预检必须考虑完整序列化请求体，禁止用 40 MiB 解码量来保证 48 MiB body 可发送。
