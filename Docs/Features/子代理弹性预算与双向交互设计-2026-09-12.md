...
> **实施状态（2026-09-12，N00，commit f096bc5；2026-09-13 token 轴重标定更新；2026-09-13 派生预算诚实化更新）**：已完成——Manager managed Math.Min(40/120)、默认32强制选择、Normalize强抬600、SubAgentInvocationContracts 三常数（DefaultWorkUnitMaxRounds/MaxWorkUnitMaxRounds/DefaultWorkUnitMaxToolCallsTotal）删除；TaskExecutionPlanCompiler 阶段小预算已完成 token 轴重标定（ADR-087 §3.2：MaxInputTokens=MaxCost×1,000,000、MaxOutputTokens=MaxRounds×4,000，六轴保持>0，token 轴降为计量轴、不再早于 rounds/cost 成为终止轴）；**派生剩余预算诚实化（原子片 2）已完成**——ExecutionUsageBudgetTracker.Remaining/SubAgentInvocationService.DividePositive 不再 Math.Max(1,…) 伪造非零（诚实归零），ExecutionUsageBudget 新增 IsDerivedRemainder（0 值轴=父级已耗尽，消除与「未设上限」的歧义）与 PeakRoundInputTokens（父级单轮峰值，仅诊断）；SubAgentExecutionOptions 新增 MinViableInputTokens/MinViableOutputTokens/MinViableCost（默认 20000/1000/0.01）与统一判据 DescribeBudgetInfeasibility，SubAgentTool 三路委派边界（批量/单代理/池化）与批量除法共用该判据，不可执行时显式拒绝（sub_agent_parent_budget_infeasible / sub_agent_batch_budget_infeasible，带数值可诊断），不再派发首轮即死的子代理。待完成（已登记的重开纠正片）——TaskExecutionPlanCompiler 阶段小预算的结构性删除（Planner 阶段预算整体移除＋单一策略对象 resolver）、AgentExecutionService.Buffered 对 >600 的 Math.Min 压制、AgentExecutionGuardrails 上界语义、SubAgentInvocationContracts.MaxRounds 的 XML 注释「Parent agents cannot override it」纠正。
