...
**稳定原因码的权威清单在源码 `Source/PuddingCore/Tools/ToolApprovalWire.cs`（单一来源，避免文档与代码漂移）；本节只定义「等待不是失败」的语义与调度动作，不重复维护码表。**
2026-09-21 新增 `CodeDeferredReasonCodeMissing`（`approval_review_deferred_reason_code_missing`）：评审器声明 `deferred_dependency` 但未给出 `reasonCode` 时，由 `ToolApprovalReviewParser` 合成该码，使「依赖等待必带稳定码」成为不变量（此前该场景 `ReasonCode` 为 null，调用方只能解析自由文本 reason）。
