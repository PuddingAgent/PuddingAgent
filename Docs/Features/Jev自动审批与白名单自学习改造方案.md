...
> ⚠️ **本文件部分条目已被取代（2026-09-20）**：请先读 `Docs/Features/安全分类器与工具调用准入方案-v2.md`。
> v2 承载用户新增需求：抽象分类器（不直接依赖 Jev）、黑白名单优先、Jev 四选一裁决
> （单次/永久 放行·拒绝）、分类器为最终裁决者、明确不依赖人工确认、Jev 下线不得影响系统。
> **已被取代**：本文 §3.1 决策映射表、§2 中「人工兜底」取向、不变量 I2（失败折叠为 `DeferredDependency`）、
> I3/I4 的「仅精确匹配」表述（按 v2 §3 重述）。**仍然有效**：§1 现状 file:line 锚点、§7 延迟实测（含 §7.5）。
...
### 7.5 连接复用对照实验（2026-09-20 补测，**推翻 §7.3 的第二条推断**）

**方法**：`temp/jev-latency-reuse.ps1` —— 复用单个 `HttpClient` + `HttpClientHandler`
（先做 1 次 warm-up 建立 keep-alive 连接，不计入统计），随后连发 8 次真实决策调用。
这正是生产路径（`IHttpClientFactory` 命名客户端）的行为。
踩坑记录：Windows PowerShell 5.1 **不自动加载** `System.Net.Http`，须先 `Add-Type -AssemblyName System.Net.Http`，
否则 `[System.Net.Http.HttpClientHandler]::new()` 报 `TypeNotFound`。

| 次 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 |
|---|---|---|---|---|---|---|---|---|
| ms | 1335 | 1398 | 1562 | 1699 | 1114 | 1130 | 1148 | **1089** |

`REUSE_MS_MIN=1089` / `MEDIAN=1335` / `MAX=1699` / `AVG=1309.4`；8/8 成功，`model=jev-1.13.0`，`inTok=286`。

**结论（更正）**
- 连接复用**并未降低延迟**：稳定段（n=5–8）为 **1089–1148 ms**，与此前「每次新建连接」的
  中位数 **1095 ms 基本一致**（差值在噪声内）。
- ⇒ §7.3 中「生产走 `IHttpClientFactory` 复用连接后**预计 600–800 ms**」的推断**被实测推翻**。
  真实预期应取 **≈1.1 s**。
- ⇒ **连带修正归因**：对照实验中 ~0.5 s 的差额**不能归因于 TLS 握手**（否则复用应当省下它）。
  更合理的解释是**服务端处理时间占主导**（托管端点自身耗时），客户端连接建立开销很小。
  （说明：本轮**仍未单独测量**纯 TLS 握手耗时；此条为**推断**，非证据。）
- ⇒ **方差很大**（1089–1699 ms，同一进程内连续调用）⇒ **单次采样不可信**，
  阈值设定与超时预算必须留余量，不得按最小值调优。

**设计含义（加强）**
1. 审批路径应把 **≈1.1 s** 当作 Jev 决策的**既定成本**来设计；3 s 超时留约 **2.7×** 余量，判定合理。
2. 白名单缓存（I6）的必要性**再获支撑**——它不是"省一点"，而是决定该特性可用与否。
3. 若要进一步压延迟，方向是**换端点**（直连 `console.typesafe.ai`），
   而非修改客户端连接策略（已证明无效）。
