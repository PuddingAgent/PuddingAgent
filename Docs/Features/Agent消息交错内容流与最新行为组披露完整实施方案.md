...
## 15. 2026-09-15 长消息阅读密度修订

本节依据用户“消息、思考和工具混杂，太长不适合阅读”的反馈修订旧的永久平铺规则，优先于本文此前相关披露描述；其余 canonical、唯一正文源和 viewport 边界不变。

### 15.1 阅读规则

- 正文按实际渲染高度判断：超过 440px 时预览 360px，末尾淡出并提供“展开完整正文”；展开后顶部与底部均可收起。短正文无额外按钮，代码、表格、图片晚到或容器宽度变化由 ResizeObserver 重新测量。
- 单回合达到 8 个交错内容块后，就地收拢连续较早前缀，保留最新行为组、其前一段播报和后续回复；展开按原序恢复，沿用每次 40 块的有界挂载。入口标注含失败记录，不把隐藏内容当作不存在。
- 用户手动展开的行为组和运行中尚有工具/委派的旧组不会被新增的阅读收拢隐藏。用户打开较早过程后，后续事件和完成状态不抢夺选择。
- 最新行为组仍默认展开，但初次只挂载最近 6 项，较早成员每次增加 24 项。静态过程超过 320px 时预览 240px，支持“展开完整过程”。
- 流式正文和正在执行的尾部行为组不做高度裁剪，避免隐藏当前输出；结束后采用静态阅读规则。
- 所有折叠仅作用于展示；完整 Markdown 不做字符串截断，复制/TTS 继续使用完整原文，canonical 事件及工具输入输出不改写。
- 按钮具备 aria-expanded/aria-controls；键盘焦点进入裁剪区内的链接或工具控件时自动展开。组件不调用 scrollTop/scrollIntoView，继续交给现有 viewport 处理高度变化。

### 15.2 实现与验证

`ExpandableMessageContent.tsx` 负责高度预览，`TextSegmentView` 和 `AgentMessageBubble` 的历史回退正文复用它；`ActivityGroup` 负责最近成员窗口与过程预览；`TurnContentStream` 负责连续前缀披露。

本次属于前端静态资源修订，无需 Core 重启，不代表本方案其余生产验收项完成。回归与发布边界见 [长消息卡片阅读优化记录](../Reports/长消息卡片阅读优化-2026-09-15.md)。
