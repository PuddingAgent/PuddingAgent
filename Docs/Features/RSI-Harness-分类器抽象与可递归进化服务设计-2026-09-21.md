...
8 | **G7 提炼契约 + C1–C5 门禁** | 整理 | 未做（**任务书已立**：`Docs/Features/RSI-G7-任务书-2026-09-22.md` —— 实测锚点 A–F（G6 现状/provenance/关键词/L3-a 货币/G3 同形/测试落点）+ 冻结契约（P1–P6 反笔记不变式、C1–C5、冷启动降级、策略对象）+ 五条“必须拒绝”负例 + 反模式禁令。⚠️ 该任务书实测发现两处与设计措辞不符，见其 §9：**“关键词唯一性守卫”写入侧不存在**（C3 实为新建）、**C4 在生产路径上恒为“记录不阻断”**） | 非工作时段 |
