...
**G4** | 家族归类 + 家族内合并（分类器） | 家族内上限强制生效；关键词唯一性守卫拒绝先到先得；合并须确定性四条件同时满足。⚠️ **口径修订（2026-09-22，D6 实测）**：四条件判据在真实语料上通过 **17/4/0** 对、叠加既有闸门后 **0/0/0** ⇒ **D1–D6c 尚未构成“实际降 N”的能力**（放行/让渡既有闸门属 D7）；另：**“关键词唯一性守卫”在写入侧并不存在**（G7 任务书 §9 实证，C3 实为新建），本轨道按“新建 + 归属裁决”落地 |
...
9 | **G4 家族归类 + 家族内上限** | 治理 | 🟡 **D1–D6c 已交付 + D9/D10 文档收口**（2026-09-22：D1 `0023baa4`、D2 `2057bba5`、D3 `67a87313`、D4a `a5406a78`、D4b `1b5daf30`、D4c `af558df7`、D5 `14e09b3`、D6a `608f6b4`、D6b `4bfedf0`、D6c `4025b9a`、探针钉基线 `86905f2`）；**D7 最薄接线未做** ⇒ 判据已备但**尚未接入编排器**（可行性裁决见下方 blockquote）；D8 用例随各片交付 | 非工作时段 |
...
> - ⇒ **当前切片（2026-09-22 06:5x 复核）= 第 9 项 `G4` 的剩余交付物**：`D1–D6c` 与 `D9/D10` 已交付（提交见第 9 行）、
>   任务书 `§4.1` 已给出逐条状态/证据/偏差/遗留，**只剩 `D7 最薄接线`**（尾随可选参数默认 `null` ⇒ 零回归；
>   家族评审计入报告字段（默认 0）；`metadata` 可观测）以及 `D8` 中依赖接线的 I8 取红。
...
>   ⇒ **“实际降 N”的收益不得记在 D1–D6c 头上**（放行/让渡既有闸门属 D7）。基线已钉入
