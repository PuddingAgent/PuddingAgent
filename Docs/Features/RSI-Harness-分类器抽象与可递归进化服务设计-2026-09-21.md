...
> **⚠️ §13.6 口径修订（2026-09-22，G4 D1–D6c 实测后）**：本节三条口径在实现时被逐条校正，⛔ 后续读者不得再按本节原文理解：
> ① **「关键词唯一性守卫」在写入侧并不存在** —— 实证见 `Docs/Features/RSI-G7-任务书-2026-09-22.md` §9（C3 实为**新建**能力，不是对既有守卫的收编）；
>    本轨道按「**只读事实暴露（D4）→ 显式裁决判据（D5）**」落地，默认**不是**先到先得、也**不是**后来者一律拒绝。
> ② **「仅适度放宽 0.35 文本相似度」被实测证伪为不够** —— D6b 在真实语料上：同族对 75 对中，程序性文本相似度 ≥0.35 的仅 **4** 对、
>    过四条件 **4** 对；≥0.20 时 **17** 对；再叠加**既有确定性闸门**后**全部为 0**。
>    ⇒ 瓶颈不在阈值松紧，而在**既有闸门本身**与**证据可归并**那一条：「放宽阈值」在不动既有闸门的前提下**无法**产生任何合并 ⇒ 该取向已被实测否决。
> ③ **分类器「家族分」未按本节落地为 LLM 分布打分** —— D1 落成**确定性聚类**（`SkillFamilyClustering`：并查集 + 名称分词相似度），
>    理由：家族键要进入上限计数 ⇒ 必须**可复现**（§13.10 R6 的风险约束）；LLM 家族分若要引入只能是**旁路标签**，⛔ 不得作为计数键。
> ④ 本节「强制进入合并评审」的**后果仍然只有评审请求**（D3 已钉住：超限分支**零写盘**、⛔ 不禁用/不删除）。
...
**G4** | 家族归类 + 家族内合并（分类器） | 家族内上限强制生效；关键词唯一性守卫拒绝先到先得；合并须确定性四条件同时满足。⚠️ **口径修订（2026-09-22，D6 实测）**：四条件判据在真实语料上通过 **17/4/0** 对、叠加既有闸门后 **0/0/0** ⇒ **D1–D6c 尚未构成“实际降 N”的能力**（放行/让渡既有闸门**原列 D7；2026-09-22 复核后移出 D7** —— D7 只交付接缝与计数可观测，放行既有闸门是语义变更且当前无收益证据 ⇒ 留待独立裁决）；另：**“关键词唯一性守卫”在写入侧并不存在**（G7 任务书 §9 实证，C3 实为新建），本轨道按“新建 + 归属裁决”落地 |
...
| 8 | **G7 提炼契约 + C1–C5 门禁** | 整理 | ✅ **已完成**（2026-09-22，commit `d89b1473`；交付物 1–10 逐条状态、验收命令与实测数字、理由码 10 条 + 降级码 3 条见任务书 **§4.1**；**设计措辞两处与磁盘不符**（关键词唯一性守卫写入侧不存在、C4 生产路径恒记录不阻断）见其 §9）—— 历史记录（**任务书已立**：`Docs/Features/RSI-G7-任务书-2026-09-22.md` —— 实测锚点 A–F（G6 现状/provenance/关键词/L3-a 货币/G3 同形/测试落点）+ 冻结契约（P1–P6 反笔记不变式、C1–C5、冷启动降级、策略对象）+ 五条“必须拒绝”负例 + 反模式禁令。⚠️ 该任务书实测发现两处与设计措辞不符，见其 §9：**“关键词唯一性守卫”写入侧不存在**（C3 实为新建）、**C4 在生产路径上恒为“记录不阻断”**） | 非工作时段 |
9 | **G4 家族归类 + 家族内上限** | 治理 | 🟡 **D1–D7 已交付 + D9/D10 文档收口**（2026-09-22：D1 `0023baa4`、D2 `2057bba5`、D3 `67a87313`、D4a `a5406a78`、D4b `1b5daf30`、D4c `af558df7`、D5 `14e09b3`、D6a `608f6b4`、D6b `4bfedf0`、D6c `4025b9a`、探针钉基线 `86905f2`）；**D7 `2b78187b`**。D7 = 尾随可选参数默认 `null` ⇒ 零回归 + `FamilyReviewCount`（默认 0）+ 作业结果 `metadata` 可观测（**只有接缝、尚未激活**：生产侧无策略来源 ⇒ 计数恒 0 是合法状态）；**可行性裁决不变**（叠加既有闸门 0/0/0 ⇒ 不主张「实际降 N」）；**D8 已交付**（2026-09-22，commit `afac60a`：I1–I9 逐条取红复核 + 补 2 条缺牙断言；取红证据表见任务书 §5.1） | 非工作时段 |
10 | **G8 触发与节奏**（定时 + 条件触发 + 非工作时段优先） | 整理 | 🟡 **任务书已立**（2026-09-22：`Docs/Features/RSI-G8-任务书-2026-09-22.md` —— 实测锚点 A–K + 不变式 P1–P9 + 切片 D1–D6）。⚠️ 该任务书实测三条硬事实：**①条件触发在生产默认下结构性不可达**（唯一构造点 `ZeroRegressionDefault(实测启用数, headroom=20)` ⇒ `softTarget = 启用数 + 20` ⇒ `enabled > softTarget` 恒不成立）；**②潜意识域不存在任何“非工作时段”概念**（off-peak 只在任务派发域与 LLM 峰谷定价域）⇒ 属**新建概念、默认关闭**；**③潜意识队列无 `Priority` 字段**（FIFO by `CreatedAt`）⇒ 设计验收语「优先级确实提升」需**单独裁决**。⇒ 本片只交付**机制 + 零回归**，⛔ 不得宣称“条件触发已生效 / 重活已落在非工作时段” | 任意 |
...
> - ⇒ **合并序列的下一个切片（2026-09-22 04:0x 复核）= 第 9 项 `G4 家族归类 + 家族内上限`**。
> - ⇒ **当前切片（2026-09-22 08:0x 复核）= 第 9 项 `G4` 已无「本片交付物」在途**：`D1–D7` 与 `D9/D10` 已交付（提交见第 9 行），
>   任务书 `§4.1` 逐条给出状态/证据/偏差/遗留。**仍开放的不是交付物而是裁决**：①「放行/让渡既有闸门」需独立裁决
>   （原被误挂在 `D7` 名下，已更正）；②生产侧尚无家族策略来源 ⇒ `D7` 的接缝**尚未激活**；③`D8` 的 I1–I9 逐条取红未复核。
>   ⇒ **合并序列的下一个切片 = 第 10 项 `G8`**。
>   ⇒ **G8 任务书已立（2026-09-22）**：`Docs/Features/RSI-G8-任务书-2026-09-22.md`。
>   **进展（2026-09-22 13:3x 复核，本节优先于上一行的「下一步 = G8-D1」）**：
>   `D1` 节奏策略对象已交付（`76477bfe`；`~Configuration` 67/67）、`D2` 非工作时段判定器已交付
>   （`d19d5c64`；`~Configuration` **79/79**，4 组变异逐条取红）。
>   ⚠️ **D3 前置盘查结论（2026-09-22 14:0x，详见任务书 §4.4）**：周期作业速率由**幂等键里的 bucket 除数**
>   （= 配置间隔）决定，`Task.Delay` 只决定窗口内“何时尝试”，且 `FindLatestAsync` / `EnqueueAsync` 均
>   **按 key 去重且不过滤 Status** ⇒ **只缩短 delay 是空操作**；要真加速必须同时换 bucket 除数 = 引入第二套口径
>   （违反已冻结的 P6）。⇒ **显式裁决：本切片只交付「判据 + 提案语义的记录」，不改变任何作业频率**；
>   **下一步 = `D3` 决定记录通道（提案命名）→ `D4`/`D5` 降级为记录+标注未生效**；真正加速需先有度量并显式修改 P6。
>   ⚠️ 本片**尚未接线**：不改调度器、不改 `SubconsciousSchedulingOptions`、零写盘 ⇒ ⛔ 不得宣称
>   「非工作时段已生效 / 重活已落在非工作时段 / 条件触发已生效 / 实际降 N」。
> - ⚠️ **G4 可行性裁决（D6b 实测，必须与第 9 行一起读，2026-09-22）**：真实技能仓（时代子集 144 技能 / 139 启用 /
>   10 家族 / 75 同族对）上，四条件判据通过 **17 / 4 / 0** 对（程序性文本阈值 0.20 / 0.35 / 0.50），
>   再叠加**既有确定性闸门**后为 **0 / 0 / 0** ⇒ 按任务书 §2.5-3「**0 ⇒ 必须显式裁决**」的约定，
>   **显式裁决：D1–D6c 当前不构成已生效的合并能力**，家族内合并仍由既有 `IsDeterministicallyEligible` 决定
>   ⇒ **“实际降 N”的收益不得记在 D1–D6c 头上**（放行/让渡既有闸门**原列 D7，2026-09-22 复核后移出**：`D7` 只交付接缝 + 计数可观测，放行既有闸门是**语义变更**且当前**无收益证据** ⇒ 留待独立裁决）。基线已钉入
>   `Source/PuddingRuntimeTests/Services/SkillMergeEligibilityRealIndexProbeTests.cs` 常量（变红须先判口径漂移、再判数据变化）。
>   ⚠️ 本行原写“下一个 = 第 6 项 G6”，在 **G6**（`ffdaa048`）、**G3**（`2aa3e7c5` 收口）、**G7**（`d89b1473`）相继交付后**已过期**；保留此修正痕迹 ——
>   **照过期表格重做**是本会话反复出现的同类缺陷（见上方 G2/S3 两条）。
...
>
> ⛔ **上述「G4 收益下调」的论据已被同目录报告修正（2026-09-22 复核）**：该结论建立在**关键词身份**聚簇口径上，
> 它只能看见「两个技能声明了同一个关键词」，**看不见措辞不同而语义相同的重复**。
> 同一批数据改用**名称分词 token 的 Jaccard** 口径（`Docs/Reports/skill-near-duplicate-2026-09-21.md`，只读、无 LLM）后：
> 139 个启用技能中 **≥0.25 档有 10 个近重复簇 / 33 个技能（23.7%）**，最大簇 11 条全为「子代理委派 + 提交推送」类，
> 次簇 5 条「任务看板」类、再次 3 条「健康检查」类 ⇒ **语义层面确实存在可合并标的**。
> ⇒ 正确表述是「**关键词层面无重叠**（重合都发生在噪声上）、**语义层面存在近重复**」；⛔ 不得再按「零重叠」低估 G4 的收益。
> G4 的冻结契约（家族口径 / `PerFamilyCap` 语义 / 关键词归属裁决 / 合并四条件）见 `Docs/Features/RSI-G4-任务书-2026-09-22.md`。
>
