...
public sealed class TypeScriptIndexer : ICodeIndexer, ICodeIndexFileUpdater, ILanguageCodeIndexer
{
    ...
    /// condition this indexer cannot handle (Node.js missing, extraction script missing, unusable extraction
    /// output, unsupported extension, noise path) is reported as <see cref="CodeIndexStatus.Failed"/> so the
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            else
            {
                await _store.UpsertFilesAsync(
                    ...
                    [new CodeFileRecord(descriptor.WorkspaceId, descriptor.ProjectId, filePath, GetLanguage(filePath), DateTimeOffset.UtcNow)],
            ...
            _logger.LogInformation(
                "TypeScript indexed file {FilePath}: {SymbolCount} symbols, {RelationCount} relations",
        ...
        {
            _logger.LogError(ex, "TypeScript indexing of file {FilePath} failed", filePath);
            return new CodeIndexResult(false, CodeIndexStatus.Failed, $"Indexing failed: {ex.Message}",
                WorkspaceId: descriptor.WorkspaceId, ProjectId: descriptor.ProjectId, StartedAtUtc: startedAt);
        }
    }
