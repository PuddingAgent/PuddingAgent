...
public class RuntimeExecuteController : ControllerBase
{
        ...
        CancellationToken ct)
    {
        ...
        _logger.LogInformation(
            "[RuntimeAPI] Execute session={SessionId} ws={Ws} template={Template} agent={AgentInstanceId} user={UserId} hasLlmConfig={HasConfig}",
            request.SessionId, request.WorkspaceId, request.AgentTemplateId, request.AgentInstanceId ?? "(none)", request.UserId ?? "(none)", request.LlmConfig is not null);
        ...
        CancellationToken ct)
    {
        ...
        _logger.LogInformation(
            "[RuntimeAPI] Stream session={SessionId} ws={Ws} template={Template} agent={AgentInstanceId} user={UserId} hasLlmConfig={HasConfig}",
            request.SessionId, request.WorkspaceId, request.AgentTemplateId, request.AgentInstanceId ?? "(none)", request.UserId ?? "(none)", request.LlmConfig is not null);
