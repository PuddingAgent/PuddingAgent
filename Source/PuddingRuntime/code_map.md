...
| `Tools/BuiltIns/Search/SearchGrepTool.cs` | 代码文本搜索（ADR-089 U0，含 R1–R4 返工）；Lucene **仅产出去重候选路径**并按存在性/scope/完整 glob/扩展名/排除五重准入，随后读取**当前内容**重新匹配（不输出索引旧文本、不预填去重键）；候选与枚举共用 `processedFiles` 与 `RetrievalMatcher`；**入口唯一 deadline**（默认 10s 可注入）覆盖候选/枚举/扫描，行间检查取消，正则超时 → `Timeout` 绝不落 `no_match`；覆盖完整性 = `errors==0 && 子目录枚举失败==0 && !timedOut && !regexTimedOut && !enumerationTruncated && !scanBudgetExceeded && !maxResultsReached && !totalCapReached && skippedLargeFiles==0`；`max_results` 统一作用于**合并后结果集**（恰好达上限且自然结束仍为 Complete）；排除目录在枚举前裁剪；默认额外排除 `.pudding`，结果默认 20 条/16 KiB |
