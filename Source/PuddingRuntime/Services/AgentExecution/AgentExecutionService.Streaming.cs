            ...
            ct);
                ...
                foreach (var tc in assistantToolCalls)
                {
                    ...
                    if (newlyLoadedToolCount > 0)
                    {
                        ...
                        _logger.LogInformation(
                            "[AgentExec:ToolDiscovery] Stream loaded {AddedCount} and promoted {PromotedCount} tool definition(s) for next LLM round session={Session} visibleToolCount={VisibleToolCount} exposureRevision={ExposureRevision} changeReason={ChangeReason} loadedTools={LoadedTools}",
