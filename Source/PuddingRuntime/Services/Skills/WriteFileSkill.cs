...
public sealed class WriteFileSkill : ContainerSkillBase
{
    ...
    public override async Task<SkillResult> ExecuteAsync(SkillInvokeRequest request, CancellationToken ct = default)
    {
        ...
        {
            _logger.LogWarning(ex, "[WriteFileSkill] Docker sandbox unavailable, falling back to host mode for path={Path}", path);
        }

        // 降级：宿主模式写入
        return WriteFileHostMode(path, content);
    }

    /// <summary>
    /// 宿主机文件写入——Docker 不可用时的降级路径。
    /// 仅允许写入当前工作目录及子目录下的文件。
    /// </summary>
    private SkillResult WriteFileHostMode(string filePath, string content)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("[WriteFileSkill] Host write: {Path} ({Size} bytes)", filePath, content.Length);
            return new SkillResult { Success = true, Output = $"File written: {filePath}" };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[WriteFileSkill] Host write failed: {Path}", filePath);
            return Fail($"Failed to write file: {ex.Message}");
        }
    }
}
