...
/// <summary>
/// Skill 包下载与解压服务。
/// Runtime 容器内 /pudding-skills 是 <c>pudding_skills</c> named volume 挂载点；
/// 解压后 Agent 容器以 <c>pudding_skills:/skills:ro</c> 只读方式挂载同一卷。
/// </summary>
public sealed class SkillPackageDownloadService(
    IHttpClientFactory httpClientFactory,
    ILogger<SkillPackageDownloadService> logger)
{
    // 宿主机（运行时容器）上 Skill 包存储根目录，与 docker-compose 挂载路径一致
