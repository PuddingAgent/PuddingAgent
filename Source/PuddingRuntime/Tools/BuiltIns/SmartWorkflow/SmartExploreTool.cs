...
public sealed class SmartExploreTool : SmartWorkflowToolBase<SmartExploreArgs>
{
    ...
    protected override string BuildTaskPrompt(SmartExploreArgs args, ToolExecutionContext context)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("### 🧭 CODE INDEX PRIORITY — Prefer structured code-index tools over raw file operations");
        sb.AppendLine("When exploring code, ALWAYS try the structured code-index tools FIRST. They are faster, cheaper, and return richer semantic information than raw file reads. Only fall back to raw file operations when the index tools are unavailable or do not have what you need.");
        ...
        sb.AppendLine();
        sb.AppendLine("⚡ EXPLORER EMPHASIS: As an Explorer sub-agent, your DEFAULT path for any code question is the structured index toolchain (code_symbol_search → code_summary → code_outline → code_explore → code_callers/code_callees). Reach for `file_read` / `search_grep` / `list_dir` ONLY as a fallback when the index tools are unavailable, return nothing, or cannot answer the question.");
