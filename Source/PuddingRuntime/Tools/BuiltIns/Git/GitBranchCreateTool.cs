...
namespace PuddingRuntime.Services.Tools
{
    ...
    [Tool(
        ...
        description: "在仓库中创建新分支（branch）。何时用：开始新功能或修复前，从当前 HEAD 拉一条独立分支，避免直接污染主干。怎么用/坑：BranchName 必填；创建后不会自动切换，需再用 git_branch_switch 或 git_checkout 切过去；分支名不能含空格或非法字符。Create a new branch in the repository; use before starting a new feature or fix to keep main clean; BranchName is required, creation does NOT switch to it (use git_branch_switch afterwards), and names with spaces or invalid characters are rejected",
