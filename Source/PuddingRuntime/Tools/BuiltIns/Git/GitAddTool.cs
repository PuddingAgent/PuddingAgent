...
namespace PuddingRuntime.Services.Tools
{
    ...
    [Tool(
        ...
        name: "Git Add",
        description: "将文件路径暂存（stage）到 Git 索引，用于提交前准备。何时用：commit 前需要把改动加入暂存区，或只想提交部分文件时精确指定 Files。怎么用/坑：Files 必填、相对仓库根目录、可传多个；未跟踪的新文件必须先 add 才能被提交；误暂存可用 git_reset 撤销。Stage file paths into the Git index before committing; use to prepare a commit or selectively stage files; Files is required (repo-root-relative, repeatable), untracked files must be added first, and git_reset undoes staging mistakes",
