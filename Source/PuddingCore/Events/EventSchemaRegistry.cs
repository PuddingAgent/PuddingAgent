...
/// <summary>
/// 事件 Schema 定义 — 描述单个事件类型的名称、版本、类别、必填/可选字段。
/// </summary>
...
{
    /// <summary>所有已注册的事件 schema。</summary>
    ...
    static EventSchemaRegistry()
    {
        ...
        {
            if (!dict.TryAdd(def.EventType, def))
                throw new InvalidOperationException($"Duplicate event type '{def.EventType}' in EventSchemaRegistry");
        }

        Schemas = dict.ToFrozenDictionary(StringComparer.OrdinalIgnoreCase);
    }

    /// <summary>验证事件类型是否已在注册表中。</summary>
    public static bool IsValidEventType(string eventType)
    {
        return Schemas.ContainsKey(eventType);
    }

    /// <summary>获取事件的当前 schema 版本。未注册类型返回 1。</summary>
    public static int GetSchemaVersion(string eventType)
    {
        return Schemas.TryGetValue(eventType, out var def) ? def.CurrentVersion : 1;
    }
    ...
    /// </summary>
    public static SchemaCompatibilityResult CheckCompatibility(string eventType, int fromVersion, int toVersion)
    {
        if (fromVersion == toVersion)
            return new SchemaCompatibilityResult(true);

        if (!Schemas.TryGetValue(eventType, out var def))
        {
            return new SchemaCompatibilityResult(false,
                $"Unknown event type '{eventType}'. Cannot verify compatibility.");
        }
    ...
    /// <summary>构建所有已知事件 schema 定义。</summary>
    private static IEnumerable<EventSchemaDefinition> BuildAllSchemas()
    {
        // ════════════════════════════════════════════════════
        // 会话事件（17 种，来自 SessionEventTypes / SseEventTypes）
        // ════════════════════════════════════════════════════
