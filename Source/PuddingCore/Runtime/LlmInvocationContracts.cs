...
public sealed record LlmInvocationRequest
{
    ...
    /// <summary>Prefix 缓存诊断快照；用于归因缓存命中率变化，不包含 raw prompt。</summary>
    public PromptPrefixSnapshot? PrefixSnapshot { get; init; }
    /// <summary>
    /// Optional call-site override for infrastructure adapters.
    /// Used when a semantic caller, such as subconscious memory processing,
    /// needs to preserve a legacy per-call model override while still routing
    /// through the shared LLM invocation facade.
    /// </summary>
    public LlmConfig? ConfigOverride { get; init; }
}
