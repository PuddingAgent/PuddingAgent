...
/// <summary>单个 TraceId 的归因结果。<paramref name="RecordedPromptTokens"/> 来自迭代结算记账，用于交叉核对。</summary>
...
public sealed class GoodputAttributionService(PlatformDbContext db) : IGoodputAttributionService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var iterationsByTrace = iterations
            .Where(i => !string.IsNullOrWhiteSpace(i.TraceId))
            .GroupBy(i => i.TraceId!, StringComparer.Ordinal)
        ...
        var traces = matched
            ...
            .Select(g =>
            {
                var traceIterations = iterationsByTrace.TryGetValue(g.Key, out var list)
                    ? list
                    : new List<GoalIterationEntity>();
                return new GoodputTraceAttribution(
                    TraceId: g.Key,
                    Rounds: g.Select(m => m.Round).Distinct().Count(),
                    IterationCount: traceIterations.Count,
                    PendingIterations: traceIterations.Count(i => IsPending(i.Status)),
                    RecordedPromptTokens: traceIterations.Sum(i => i.InputTokens),
                    RecordedCompletionTokens: traceIterations.Sum(i => i.OutputTokens),
        ...
        var seenTraces = traces.Select(t => t.TraceId).ToHashSet(StringComparer.Ordinal);
        var iterationsWithoutUsage = iterations.Count(i =>
            string.IsNullOrWhiteSpace(i.TraceId) || !seenTraces.Contains(i.TraceId!));

        return new GoodputAttributionReport(
            GoalRunId: goalRunId,
            IterationCount: iterations.Count,
            IterationsWithoutUsage: iterationsWithoutUsage,
    ...
    /// <summary>迭代是否尚未结算（此时结算记账列必然为 0，不能与归因结果直接对比）。</summary>
