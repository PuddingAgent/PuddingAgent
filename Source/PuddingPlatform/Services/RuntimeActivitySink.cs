...
public sealed class RuntimeActivitySink : IRuntimeActivitySink
{
    ...
    public async Task RecordAsync(RuntimeActivity activity, CancellationToken ct = default)
    {
        ...
        catch (InvalidOperationException ex)
        {
            _logger.LogError(
                ex,
                "[RuntimeActivity] Invalid activity id={ActivityId} component={Component} op={Operation}; refusing to write ambiguous diagnostic data",
                activity.ActivityId,
                activity.Component,
                activity.Operation);
            throw;
    ...
    private static RuntimeActivityEntity ToEntity(RuntimeActivity activity)
    {
        var stage = RuntimePipelineStages.ResolveForActivity(
            ...
            activity.Operation,
            activity.Status);
        var metadata = RuntimePipelineStages.Enrich(activity.Metadata, stage);
