    ...
    ILogger<BootstrapApiController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> Complete([FromBody] BootstrapCompleteRequest request, CancellationToken ct)
    {
        ...
        try
        {
            ...
            await EnsureDefaultTemplateRoutingAsync(workspace.WorkspaceId, providerResult, ct);
            await EnsureDefaultAgentAsync(workspace.WorkspaceId, request.Defaults, providerResult, ct);
    ...
    private async Task EnsureDefaultTemplateRoutingAsync(
        string workspaceId,
        BootstrapProviderResult providerResult,
        CancellationToken ct)
    {
        // 模板已迁移到文件管理（AgentTemplateFileService），不再需要 DB 种子写入
        _ = workspaceId;
        _ = providerResult;
        _ = ct;
    }

    private async Task EnsureDefaultAgentAsync(
        string workspaceId,
        BootstrapDefaultsRequest? defaults,
        BootstrapProviderResult providerResult,
        CancellationToken ct)
    {
        var existingAgents = await workspaceAgentFileService.ListAgentsAsync(workspaceId, ct);
        if (existingAgents.Count > 0) return;

        var agentName = string.IsNullOrWhiteSpace(defaults?.AgentName)
            ? "默认助手"
            : defaults.AgentName.Trim();

        await workspaceAgentFileService.CreateAgentAsync(
            workspaceId,
            new CreateWorkspaceAgentRequest(
                Name: agentName,
                Description: "首次初始化创建的默认助手",
                DisplayName: agentName,
                AvatarId: null,
                AvatarUrl: null,
                SourceTemplateId: "global:general-assistant",
                SystemPromptOverride: null,
                PreferredProviderId: providerResult.ProviderId,
                PreferredModelId: providerResult.ChatModelId),
            ct);
    }
