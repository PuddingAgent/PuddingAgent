...
public sealed class AgentWorkspaceGuardTests
{
    ...
    [TestMethod]
    public void CanExecuteTool_MemorySearchInAllow_Allowed()
    {
        var (guard, _) = CreateGuard();
        // 默认 Allow 列表为空时允许所有工具
        var decision = guard.CanExecuteTool("test-agent", "memory.search");

        Assert.IsTrue(decision.Allowed);
    }

    [TestMethod]
    public void CanExecuteTool_ShellExecuteInDeny_Denied()
    {
        ...
        var decision = guard.CanExecuteTool("test-agent", "shell.execute");
