...
public sealed class TaskPlanningSchemaBootstrapperTests
{
    ...
    public async Task EnsureCreatedAsync_Creates_TaskPlanningTables_ForExistingSqliteDatabase()
    {
        ...
        await using (var db = new PlatformDbContext(options))
        {
            ...
            Assert.IsTrue(await TableExistsAsync(db, "work_unit_await_handles"));
            ...
            Assert.IsTrue(await IndexExistsAsync(db, "IX_task_nodes_plan_sequence"));
            Assert.IsTrue(await IndexExistsAsync(db, "IX_work_unit_await_handles_plan_node_status"));
            ...
            AssertColumn(taskNodeColumns, "task_nodes", "sequence_no", "INTEGER", checkNotNull: true, expectedDefault: "0");
            AssertColumn(taskNodeColumns, "task_nodes", "work_unit_kind", "TEXT", checkNotNull: false);
            AssertColumn(taskNodeColumns, "task_nodes", "depends_on_json", "TEXT", checkNotNull: false);
            AssertColumn(taskNodeColumns, "task_nodes", "scope_json", "TEXT", checkNotNull: false);
            AssertColumn(taskNodeColumns, "task_nodes", "max_rounds", "INTEGER", checkNotNull: false);
            AssertColumn(taskNodeColumns, "task_nodes", "checkpoint_artifact_ref", "TEXT", checkNotNull: false);
