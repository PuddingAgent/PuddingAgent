...
public sealed class MemoryLibraryAdr029Tests
{
    ...
    public async Task GrepMemory_ShouldNotReturnOtherWorkspace()
    {
        ...
        var requestA = CreateSkillRequest("ws-a", new Dictionary<string, string>
        {
            ["action"] = "search",
            ["query"] = "唯一关键词XYZ789",
            ["top_k"] = "10"
        ...
        var skillResult = await ((IAgentSkill)tool).ExecuteAsync(requestA);
        Assert.IsTrue(skillResult.Success, skillResult.Error);
        Assert.IsTrue(skillResult.Output!.Contains("唯一关键词XYZ789"), "ws-a should find its own chapter");

        var requestB = CreateSkillRequest("ws-b", new Dictionary<string, string>
        {
            ["action"] = "search",
            ["query"] = "唯一关键词XYZ789",
            ["top_k"] = "10"
        ...
        var skillResultB = await ((IAgentSkill)tool).ExecuteAsync(requestB);
        Assert.IsTrue(skillResultB.Success, skillResultB.Error);
        Assert.IsFalse(skillResultB.Output!.Contains("A章节"), "ws-b should NOT find ws-a chapters");
    }
    ...
    public async Task SaveMemorySkill_ShouldInjectWorkspace()
    {
        ...
        var result = await ((IAgentSkill)tool).ExecuteAsync(CreateSkillRequest("ws-tool-test", new Dictionary<string, string>
        {
            ["type"] = "fact",
    ...
    private static SkillInvokeRequest CreateSkillRequest(string workspaceId, Dictionary<string, string> parameters)
    {
        return new SkillInvokeRequest
        {
