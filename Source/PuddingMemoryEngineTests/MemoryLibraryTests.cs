...
public sealed class MemoryLibraryTests
{
    ...
    [TestMethod]
    public async Task CreateBook_ConcurrentSameTitle_ShouldReturnSingleActiveBook()
    {
        var dbPath = Path.Combine(Path.GetTempPath(), $"pudding-memory-concurrent-{Guid.NewGuid():N}.db");
        try
        {
            ...
            var lib = await library.CreateLibraryAsync("ws-1", "Lib", null);

            var writes = Enumerable.Range(0, 8)
                .Select(i => library.CreateBookAsync(lib.LibraryId, "Concurrent Book", $"summary-{i}"))
                .ToArray();
            var results = await Task.WhenAll(writes);

            var books = await library.ListBooksAsync(lib.LibraryId, 20);
            var sameTitleBooks = books.Where(b => b.Title == "Concurrent Book" && b.Status == "active").ToList();
            Assert.HasCount(1, sameTitleBooks);
            Assert.IsTrue(results.All(r => r.BookId == sameTitleBooks[0].BookId));
        }
        finally
        {
    ...
    {
        var dataPaths = PuddingCode.Configuration.PuddingDataPaths.FromRoot(Path.GetTempPath());
        return new TerminalProcessManager(NullLogger<TerminalProcessManager>.Instance, dataPaths);
    }
