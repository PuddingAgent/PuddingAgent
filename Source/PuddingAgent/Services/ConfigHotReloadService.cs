...
public sealed class ConfigHotReloadService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        void TriggerReload(string changeType, string path)
        {
            ...
            _ = delay.ContinueWith(_ =>
            {
                ...
                {
                    _logger.LogDebug("[HotReload] 忽略文件变更: {File}", fileName);
                }
    ...
    private void ReloadLlmConfig()
    {
        try
        {
            if (_llmConfig is PuddingFileLlmConfigService svc)
            {
                svc.Reload();
                _logger.LogInformation("[HotReload] LLM 配置已重新加载 (缓存已失效)");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[HotReload] LLM 配置重载失败");
        }
    }
