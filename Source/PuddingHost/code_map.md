...
| `Tools/ImageReaderTool.cs` + `Tools/ImageReaderSourceResolver.cs` | ADR-077 V2 取图工具：`path` 唯一必填（http(s) URL/宿主绝对路径/`artifact://`）；Low 权限 ReadOnly\|RequiresNetwork（2026-08-28 裁定：纯只读无写/删路径，免审直通）；2026-09-15 按 ADR-088 移除 mode/helper invocation，只返回 typed 图片给调用模型；无视觉能力或不支持工具图片传输时明确报错；URL 有界下载每跳 SSRF 重校验（禁内网）、本地只读、内容哈希稳定 `vision-*` Artifact；错误走 ADR-077 §9.1 稳定码。**V6-T3（2026-09-13）**：新增可选变换参数（`transform=zoom_in\|zoom_out\|crop\|rotate` + `scale/cropX/cropY/cropWidth/cropHeight/rotation`），由纯函数 `ImageTransforms`（`Parse` 校验 + `Apply`：SKBitmap 解码 → 缩放/裁剪/旋转 → JPEG q92/PNG 编码；`MaxSide=8192`、`MaxPixels=33,554,432`）产出**派生** `vision-*` Artifact（按新内容哈希入库，原 artifact 只读不动），附 `ImageTransformPlan` 与 native summary 的 `transformNote`；非法参数 → `vision_source_invalid`，不支持 mime（如 webp）→ `vision_media_invalid`，均 fail-closed 且不泄漏路径。运行目录需存在 SkiaSharp **托管**程序集（由 `PuddingHost.csproj` 显式 `PackageReference` 覆盖上游 NPOI 的 `exclude="Runtime"`） |
| `Hosting/PuddingApplicationInitializer.cs` | 启动期数据库初始化；包含 AppUsers、WorkspaceTask、TaskPlanning/WorkUnit/AwaitHandle、Goal 及通用编排 SQLite schema bootstrap，已有数据库也必须幂等升级；GoalSchemaBootstrapper 后执行 GoalRestartReconciler 启动 disarm（active→paused） |
