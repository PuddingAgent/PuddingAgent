...
public static class BuiltInAgentTemplates
{
    public static readonly AgentTemplateDefinition WorkspaceServiceAgent = new()
    {
        ...
        Capability = new CapabilityPolicy
        {
            ...
            DefaultToolNames = ["search_memory", "save_memory", "manage_memory", "grep_memory",
                ...
                "spawn_sub_agent", "event_subscribe", "sleep", "goal_read", "goal_update",
    ...
    public static readonly AgentTemplateDefinition WorkspaceAuditAgent = new()
    {
        ...
        Capability = new CapabilityPolicy
        {
            ...
            DefaultToolNames = ["search_memory", "grep_memory", "query_sessions", "query_session_logs", "file_read", "list_dir",
                "file_search", "search_grep", "event_subscribe", "sleep", "goal_read", "goal_update"],
            RequiresGrantToolNames = [],
        },
        Runtime = new RuntimeProfile { MaxContextTokens = 1048576 },
        Memory = new MemoryPolicy
    ...
    public static AgentTemplateDefinition ResolveBest(string rawTemplateId)
    {
        ...
        //    Extract the segment after "global_" or the meaningful template name
    ...
    /// <summary>
    /// 从复合 ID 中提取核心模板名称。
    /// "default.global_code-assistant.78ff46" → "code-assistant"
    /// "global:code-assistant" → "code-assistant"
    /// </summary>
    private static string ExtractTemplateCoreName(string id)
    {
        // Handle compound format: workspace.global_{name}.{instanceId}
        ...
        // Handle dotted format: workspace.{name}.{instanceId}
        var parts = id.Split('.');
        if (parts.Length >= 2)
        {
            // Try the second segment as the template name
            return parts[1];
        }
