...
public static partial class PuddingServiceCollectionExtensions
{
        ...
        string aspnetcoreEnvironment)
    {
        ...
        //    避免 Singleton 捕获 scoped DbContext（captive dependency）；控制器改为请求 ISkillHubService，行为零变化。
        builder.Services.AddScoped<PuddingCode.Skills.ISkillHubService, PuddingPlatform.Services.SkillHubService>();
        // SKILL Hub feature flag（配置节 SkillHub）：供 skill_hub 工具（Singleton）无感注入开关。
        builder.Services.Configure<PuddingCode.Skills.SkillHubFeatureOptions>(
            builder.Configuration.GetSection(PuddingCode.Skills.SkillHubFeatureOptions.SectionName));
        ...
        builder.Services.AddSingleton<PlatformReadinessProbe>();
            ...
            builder.Services.AddSingleton<GoalContractRefinementStore>();
        ...
        builder.Services.AddSingleton<ITaskSchedulerIntentOutcomeStore>(sp =>
            sp.GetRequiredService<TaskSchedulerIntentOutcomeStore>());
        ...
        builder.Services.AddHttpClient("DashScopeVisualReasoning", client =>
        {
            client.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent", PuddingUserAgent.Value);
        });
