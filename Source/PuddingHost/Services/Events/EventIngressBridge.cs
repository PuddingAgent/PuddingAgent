...
public class EventIngressBridge : IHostedService
{
    ...
    {
        Console.WriteLine("[Startup] EventIngressBridge.StartAsync — subscribing to event bus...");
        // 订阅所有事件类型（通配符 *）
        _subscription = await _eventBus.SubscribeAsync("*", OnEventAsync, ct);
        Console.WriteLine("[Startup] EventIngressBridge — subscribed, feeding into pipeline");
    ...
    private async Task OnEventAsync(InternalEvent evt)
    {
        try
        {
            if (IsHookLifecycleEvent(evt))
            {
                _logger.LogDebug(
                    "[EventIngressBridge] Skip hook lifecycle event id={Id} type={Type}; consumed by hook subscribers",
                    evt.EventId,
                    evt.Type);
                return;
            }
    ...
    private static bool IsHookLifecycleEvent(InternalEvent evt)
        => evt.Metadata is not null
            && evt.Metadata.TryGetValue("hook_event", out var hookEvent)
            && !string.IsNullOrWhiteSpace(hookEvent);
}
