...
| `src/pages/chat/hooks/useSessionEventConnection.ts` | **游标必须显式**（S3，2026-09-19）：`startSessionEventStream(sessionId, {cursor, reason})`；显式 0 ＝「有意全量回放」（仅 compaction 后继会话与 checkpoint fork 这类**刚创建**的会话，日志短且有界），省略 ＝ 沿用 `lastSequenceNumRef`（bootstrap/history 写入的权威位置），诊断日志记录 `cursorSource`/`cursorReason`。**410 分流**（S4 剩余项，2026-09-19）：按服务端 `code` 区分 `snapshot_required`（可恢复 → 端口 `syncCompletedHistoryEventCursor(sessionId, signal, {resetCursor:true})` 重取快照并强制重置游标后重连）与 `conversation_frozen`/404（终态 → `handleSessionNotFound`） |
| `src/services/platform/api.ts` | `subscribeSessionEvents` **任何** `afterSequence`（含 0）都下发 `Last-Event-ID`（S3，2026-09-19）——此前只在 `>0` 时发送，使「显式 0（有意全量回放）」与「无游标（客户端没有权威位置）」在协议上不可区分，而无游标已被服务端改为只推实时帧（fail-safe） |
