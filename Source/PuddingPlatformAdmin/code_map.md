...
| `src/pages/chat/hooks/useSessionEventConnection.ts` | **游标必须显式**（S3，2026-09-19）：`startSessionEventStream(sessionId, {cursor, reason})`；显式 0 ＝「有意全量回放」（仅 compaction 后继会话与 checkpoint fork 这类**刚创建**的会话，日志短且有界），省略 ＝ 沿用 `lastSequenceNumRef`（bootstrap/history 写入的权威位置），诊断日志记录 `cursorSource`/`cursorReason` |
