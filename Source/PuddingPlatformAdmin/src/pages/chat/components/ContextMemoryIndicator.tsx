...
const ContextMemoryIndicator: React.FC<ContextMemoryIndicatorProps> = ({
  ...
  const uniqueId = useMemo(
    () => `ig_${Math.random().toString(36).slice(2, 8)}`,
    [],
  );
              ...
              <>
                <div className={styles.popupRow}>
                  ...
                  <span className={styles.popupValue}>
                    {tLimit > 0
                      ? `${(tUsed / 1024).toFixed(1)}K / ${(tLimit / 1024).toFixed(0)}K`
                      : '模型未配置'}
                  </span>
                </div>
                <div className={styles.popupProgress}>
                  <div
                    className={styles.popupProgressBar}
                    style={{
                      width: `${tPct}%`,
                ...
                <div className={styles.popupRow}>
                  <span>模型上下文</span>
                  <span className={styles.popupValue}>
                    {tLimit > 0 ? `${(tLimit / 1024).toFixed(0)}K` : '未知'}
                  </span>
                </div>
                <div className={styles.popupRow}>
                  <span>已使用</span>
                  <span className={styles.popupValue}>
                    {tUsed > 0 ? `${(tUsed / 1024).toFixed(1)}K` : '0'}
                  </span>
                </div>
                {tPct > 80 && (
