...
import { EARTH_LAND_POLYGONS, type EarthLandPolygon, type LonLat } from './earthVectorData';
...
interface PreparedLandPolygon extends EarthLandPolygon {
  readonly bbox: readonly [minLon: number, minLat: number, maxLon: number, maxLat: number];
}
...
const COAST_SEGMENTS: readonly CoastSegment[] = PREPARED_LAND_POLYGONS.flatMap((polygon) => {
...
const TOTAL_COAST_WEIGHT = COAST_SEGMENTS.reduce((sum, segment) => sum + segment.weight, 0);
...
function pointInRing(lon: number, lat: number, ring: readonly LonLat[]): boolean {
  ...
  for (let i = 0, j = ring.length - 1; i < ring.length; j = i++) {
    ...
    const intersectLon = ((xj - xi) * (lat - yi)) / (yj - yi + Number.EPSILON) + xi;
...
function toAngles(lat: number, lon: number): Pick<GlobeParticle, 'theta' | 'phi'> {
  return {
...
function createParticle(kind: GlobeKind, lat: number, lon: number, alpha: number): GlobeParticle {
  const { theta, phi } = toAngles(lat, lon);
  return { kind, theta, phi, alpha, px: Number.NaN, py: Number.NaN, vx: 0, vy: 0 };
}

function createSurfaceParticle(kind: 'land' | 'ocean', random: () => number): GlobeParticle | undefined {
  for (let attempt = 0; attempt < MAX_SAMPLE_ATTEMPTS; attempt++) {
    ...
    if ((kind === 'land' && isLand) || (kind === 'ocean' && !isLand)) {
      const alpha = kind === 'land' ? 0.42 + random() * 0.34 : 0.15 + random() * 0.18;
      return { kind, theta, phi, alpha, px: Number.NaN, py: Number.NaN, vx: 0, vy: 0 };
    }
  }
...
function createCoastParticle(random: () => number): GlobeParticle {
  ...
  const t = random();
  const jitter = 0.35;
  const lon = selected.a[0] + (selected.b[0] - selected.a[0]) * t + (random() - 0.5) * jitter;
  const lat = selected.a[1] + (selected.b[1] - selected.a[1]) * t + (random() - 0.5) * jitter;

  return createParticle('coast', lat, Math.max(-180, Math.min(180, lon)), 0.66 + random() * 0.22);
}
...
function createGlobeParticles(): GlobeParticle[] {
  ...
  while (particles.filter((particle) => particle.kind === 'land').length < LAND_PARTICLE_COUNT) {
    const particle = createSurfaceParticle('land', random);
    if (!particle) break;
    particles.push(particle);
  }

  while (particles.filter((particle) => particle.kind === 'ocean').length < OCEAN_PARTICLE_COUNT) {
...
function parseCssColor(variableName: string, fallback: [number, number, number]): [number, number, number] {
...
const GlobeSphere: React.FC<GlobeSphereProps> = ({ size = 300, className, style }) => {
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      const projected = particlesRef.current.map((particle) => {
        ...
        return { x: particle.px, y: particle.py, z: target.z, alpha: particle.alpha, kind: particle.kind };
      });
  ...
  return <canvas ref={canvasRef} className={className} style={{ cursor: 'pointer', display: 'block', ...style }} />;
};
