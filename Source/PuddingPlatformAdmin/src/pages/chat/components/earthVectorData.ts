...
function extractExteriorRings(geometry: GeoGeometry): readonly (readonly LonLat[])[] {
...
function buildLandPolygons(): readonly EarthLandPolygon[] {
  ...
  const geo = feature(landTopology as never, topology.objects.land as never) as unknown as GeoFeature | GeoFeatureCollection;
  const geometries = geo.type === 'FeatureCollection' ? geo.features.map((item) => item.geometry).filter(Boolean) : [geo.geometry];
  ...
  for (const geometry of geometries) {
    ...
    for (const ring of extractExteriorRings(geometry)) {
      ...
      polygons.push({ name: `natural-earth-land-${polygons.length + 1}`, ring });
    }
  }
...
export const EARTH_LAND_POLYGONS: readonly EarthLandPolygon[] = buildLandPolygons();
