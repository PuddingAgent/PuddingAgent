...
const IndexIndicator: React.FC<IndexIndicatorProps> = ({ active, fileCount, statusText }) => {
  const { styles } = useChatStyles();

  return (
    <Tooltip title={statusText || (active ? `索引已激活 · ${fileCount ?? '—'} 个文件` : '索引未启动')}>
      <span className={styles.statusIconGroup}>
        <SearchOutlined className={styles.statusIconThunder} style={{
          fontSize: 12,
