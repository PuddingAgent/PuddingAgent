...
const WorkspaceAgentTemplatePage: React.FC = () => {
  ...
  useEffect(() => {
    history.replace(buildWorkspaceSettingsPath('default', 'templates'));
  }, []);
  ...
  return (
    <PageContainer
      ...
      extra={(
        <Button type="primary" onClick={() => history.push(buildWorkspaceSettingsPath('default', 'templates'))}>
          前往场景设置
        </Button>
      )}
    />
  ...
  const handleEmbeddingProviderChange = async (providerId: string) => {
    ...
    if (!providerId) { setEmbeddingModels([]); return; }
