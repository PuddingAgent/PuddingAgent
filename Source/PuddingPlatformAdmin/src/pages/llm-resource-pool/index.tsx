...
const ModelTable: React.FC<{ provider: LlmProviderDetailDto; onRefresh: () => void }> = ({
  ...
  const openEdit = (m: LlmModelDto) => {
    setEditModel(m);
    modelForm.setFieldsValue(modelToFormValues(m));
    setModelDrawer(true);
  };

  const handleSave = async () => {
    const values = modelFormValuesToApiValues(await modelForm.validateFields());
    if (editModel) {
  ...
  const columns: ProColumns<LlmModelDto>[] = [
    ...
    },
    {
      ...
      width: 130,
      render: (_, r) => formatPricePer1MTokensRmb(r.inputPricePer1MTokens),
    },
    {
      title: '输出价格 (RMB/1M)',
      dataIndex: 'outputPricePer1MTokens',
      width: 130,
      render: (_, r) => formatPricePer1MTokensRmb(r.outputPricePer1MTokens),
    },
    {
      title: '缓存命中价格 (RMB/1M)',
      ...
      render: (_, r) =>
        r.cacheHitPricePer1MTokens > 0
          ? formatPricePer1MTokensRmb(r.cacheHitPricePer1MTokens)
    ...
    <>
      ...
      >
        <ProForm form={modelForm} submitter={false} layout="vertical">
          ...
          <ProFormDigit name="outputPricePer1MTokens" label="输出价格 (RMB/1M tokens)"
            fieldProps={{ precision: 6 }} />
          <ProFormDigit name="cacheHitPricePer1MTokens" label="缓存命中价格 (RMB/1M tokens)"
            fieldProps={{ precision: 6 }}
            tooltip="缓存命中的 prompt tokens 计费单价（RMB/百万 tokens），0 表示与输入价格相同"
            placeholder="0" />
          <ProFormSelect
            name="capabilityTags"
            label="能力标签"
          ...
          <Space>
            ...
            <ProFormSwitch name="isEmbedding" label="Embedding 模型" />
          </Space>
          <ProFormDigit name="sortOrder" label="排序权重" min={0} />
          <ProFormDigit name="maxConcurrentRequests" label="最大并发请求数"
            min={0} max={9999}
            tooltip="针对该模型的最大并发请求数限制，留空则使用服务商级别默认值"
            placeholder="留空使用服务商默认值" />
        </ProForm>
      </Drawer>
    </>
  );
...
const LlmResourcePoolPage: React.FC = () => {
  ...
  const [providerTemplateValue, setProviderTemplateValue] = useState<string>();
  ...
  const handleProviderTemplateChange = (value?: string) => {
    setProviderTemplateValue(value);
    const template = LLM_PROVIDER_TEMPLATES.find((item) => item.value === value);
    if (!template) return;
    providerForm.setFieldsValue(getProviderTemplateProviderValues(template));
  };
  ...
  const handleSaveProvider = async () => {
    ...
    if (editProvider) {
      ...
      const template = LLM_PROVIDER_TEMPLATES.find((item) => item.value === providerTemplateValue);
      const templateModels = template ? getProviderTemplateModelValues(template) : [];
      for (const model of templateModels) {
        await createLlmModel(values.providerId, model);
      }
      message.success(
        templateModels.length > 0
          ? `服务商已创建，已添加 ${templateModels.length} 个模型`
          : '服务商已创建',
      );
    }
        ...
        <ProForm form={providerForm} submitter={false} layout="vertical">
          {!editProvider && (
            <Form.Item label="服务商模板">
              <Select
                allowClear
                value={providerTemplateValue}
                options={LLM_PROVIDER_TEMPLATES.map((template) => ({
                  label: template.label,
                  value: template.value,
                }))}
                placeholder="选择预设模板"
                onChange={handleProviderTemplateChange}
              />
            </Form.Item>
          )}
          <ProFormText
