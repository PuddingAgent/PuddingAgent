...
public sealed class RuntimeRegistryService
{
    ...
    public RuntimeRegisterResponse Register(RuntimeRegisterRequest request)
    {
        ...
        _logger.LogInformation("[RuntimeRegistry] Registered node={NodeId} endpoint={Endpoint} activeSessions={Count}",
            request.NodeId, request.Endpoint, request.ActiveSessionCount);

        return new RuntimeRegisterResponse { Accepted = true, Message = "OK" };
    }
