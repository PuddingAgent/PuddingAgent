...
public sealed class SessionRouter
{
    ...
    public async Task<MessageIngressResponse> RouteMessageAsync(MessageIngressRequest request, CancellationToken ct = default)
    {
        ...
        if (session is null && request.SessionId is null && !request.ForceNewSession)
        {
        ...
        if (session is null)
        {
            session = new SessionRecord
            {
                ...
                ChannelId = request.ChannelId,
                OwnerUserId = userId,
                Title = ResolveInitialSessionTitle(request),
            };
        ...
        var dispatchRequest = new RuntimeDispatchRequest
        {
            ...
            MessageId = messageId,
            UserId = request.UserId ?? request.UserExternalId,
            AgentInstanceId = request.AgentInstanceId
                ?? request.Metadata?.GetValueOrDefault("agent_id"),
            LlmConfig = request.LlmConfig,
            CapabilityPolicy = request.CapabilityPolicy,
            ToolDefinitions = request.ToolDefinitions,
            SkillPackages = request.SkillPackages,
            TaskPlanId = GetMetadataValue(request.Metadata, "task_plan_id", "taskPlanId", "TaskPlanId"),
            TaskNodeId = GetMetadataValue(request.Metadata, "task_node_id", "taskNodeId", "TaskNodeId"),
            ParentTaskNodeId = GetMetadataValue(request.Metadata, "parent_task_node_id", "parentTaskNodeId", "ParentTaskNodeId"),
            DelegationDepth = GetMetadataInt(request.Metadata, "delegation_depth", "delegationDepth", "DelegationDepth"),
            MaxDelegationDepth = GetMetadataInt(request.Metadata, "max_delegation_depth", "maxDelegationDepth", "MaxDelegationDepth"),
            RoleInPlan = GetMetadataValue(request.Metadata, "role_in_plan", "roleInPlan", "RoleInPlan"),
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        yield return ServerSentEventFrame.Json(SseEventTypes.Metadata, new
        {
            ...
            agent_id = request.Metadata?.GetValueOrDefault("agent_id"),
            audience = request.Metadata?.GetValueOrDefault("audience"),
            target_agent_ids = request.Metadata?.GetValueOrDefault("target_agent_ids"),
            avatar_url = request.Metadata?.GetValueOrDefault("avatar_url"),
            fanout_index = request.Metadata?.GetValueOrDefault("fanout_index"),
            fanout_count = request.Metadata?.GetValueOrDefault("fanout_count"),
        });
        ...
        await foreach (var frame in _dispatcher.DispatchStreamAsync(dispatchRequest, ct))
            yield return EnsureFrameMessageId(frame, messageId);
    ...
    private static ServerSentEventFrame EnsureFrameMessageId(ServerSentEventFrame frame, string messageId)
    {
        ...
        {
            // Preserve malformed frame data rather than breaking the stream.
        }

        return frame;
    }

    private static string? ResolveInitialSessionTitle(MessageIngressRequest request)
    {
        if (!string.IsNullOrWhiteSpace(request.SessionTitle))
            return request.SessionTitle.Trim();

        if (request.Metadata is not null
            && request.Metadata.TryGetValue("source_name", out var sourceName)
