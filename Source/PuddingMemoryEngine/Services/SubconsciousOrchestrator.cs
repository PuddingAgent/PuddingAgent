...
5. Max 5 operations. Output JSON: {""operations"":[{""kind"":""merge|archive|delete"",""reason"":""..."",""bookId"":""..."",""sourceBookId"":""..."",""priority"":1}]}";
        ...
        CancellationToken ct = default)
    {
        ...
        {
            _logger.LogInformation(
                "[SkillCuration] Read-only curation scan workspace={WorkspaceId} agent={AgentInstanceId}",
                workspaceId,
                agentInstanceId);

            var allSkills = await _skillStore.ListAutoGeneratedAsync(agentInstanceId, ct);
            var enabledSkills = allSkills.Where(skill => skill.Enabled).ToList();
            var nBefore = enabledSkills.Count;

            // K：提炼候选 = 尚无当前评估标记的启用技能（与 ImproveSkills Phase1 筛选同形，但只计数不消费）。
            var candidateCount = enabledSkills
                .Count(skill => !SkillEvolutionDeduplicationService.HasCurrentEvaluation(skill));

            // R：淘汰建议 = 同名冗余（同 Name 多个启用技能，除每组首个外均为重复候选；只读确定性判据）。
            var retireSuggestionCount = enabledSkills
                .GroupBy(skill => skill.Name, StringComparer.Ordinal)
                .Sum(group => group.Count() - 1);

            // I1：本片零写盘 ⇒ n_after 必须等于 n_before。
            // 度量纪律：n_after 必须**在扫描之后重新从存储读取**；若沿用上面同一内存列表重算，
            // 该字段就恒等于 n_before（整数恒等式），防线无牙 —— 越界写盘也永远看不出来。
            var afterScan = await _skillStore.ListAutoGeneratedAsync(agentInstanceId, ct);
            var nAfter = afterScan.Count(skill => skill.Enabled);
            if (nAfter != nBefore)
            {
                _logger.LogWarning(
                    "[SkillCuration] Enabled skill count changed during curation scan {NBefore} -> {NAfter} " +
                    "workspace={WorkspaceId} agent={AgentInstanceId}; curation itself is report-only.",
                    nBefore,
                    nAfter,
                    workspaceId,
                    agentInstanceId);
            ...
            _logger.LogInformation(
                ...
                nAfter,
                candidateCount,
                retireSuggestionCount);

            return new SkillCurationReport
            {
                DurationMs = sw.ElapsedMilliseconds,
                NBefore = nBefore,
                NAfter = nAfter,
        ...
        {
            _logger.LogWarning(ex, "[SkillCuration] Curation scan failed workspace={WorkspaceId}", workspaceId);
            return new SkillCurationReport
            {
                DurationMs = sw.ElapsedMilliseconds,
                NBefore = 0,
                NAfter = 0,
                NotReducedReason = $"Curation scan failed before any skill could be touched: {ex.Message}",
                CandidateCount = 0,
                RetireSuggestionCount = 0,
                Summary = "Curation scan failed; report-only contract kept, no skills modified",
                Timestamp = DateTime.UtcNow
            };
        }
    }
