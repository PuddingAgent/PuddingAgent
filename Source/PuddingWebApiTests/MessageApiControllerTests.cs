...
public sealed class MessageApiControllerTests
{
    ...
    public async Task ChatTranscriptWriter_PersistsUserAndAgentMessages_WithIdempotency()
    {
        ...
        await writer.PersistMessageAsync(sid, "user", "hello", createdAt);
        await writer.PersistMessageAsync(sid, "agent", "world", createdAt + 10, usageJson: "{\"totalTokens\":3}");

        // 同一窗口内重复写入同一内容应被忽略，避免后台重试产生重复转录。
        await writer.PersistMessageAsync(sid, "agent", "world", createdAt + 20, usageJson: "{\"totalTokens\":3}");
