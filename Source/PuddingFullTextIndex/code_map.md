...
## 变更（2026-09-26，S3d：维护循环本体 + `ProbeIntegrityAsync` 落地 + 裁定③ · 组件内 · 未接宿主）

- **新增生产（3）**：`Infrastructure/Maintenance/LuceneFullTextIndexMaintenance.cs`（**1913 行**，`IFullTextIndexMaintenance` 真实实现）· `MaintenanceCorpusScan.cs`（126，维护侧共用语料走查器）· `IResourcePressureProbe.cs`（135，压力采样接缝 + 默认 CPU 探针）。
- **修改生产（2，只增不改）**：`LuceneFullTextIndexMaintenanceEngine.cs`（1031 → **1207**：**只**把 `ProbeIntegrityAsync` 从 `NotSupportedException` 换成只读三态探针 + 一个私有 `MeasureDirectoryBytesOrNull`；既有条目逐字未动）· `MaintenanceOptions.cs`（453 → **476**：+`RetainedOldWarnThreshold`（默认 3，区间 `1..MaxRetainedOldWarnThreshold`=1000）+ 界常量 + fail-closed 校验，**与开关无关始终校验**）。
- **新增测试（3 / +19 用例）**：`MaintenanceLoopTests.cs`（746，M1~M6/M9~M12）· `MaintenanceLoopHarness.cs`（474，`FakeTimeProvider` / `FixedPressureProbe` / `ScriptedFactsExtractor` / `GatedExtractor` / `RecordingMaintenanceEngine`）· `MaintenanceIntegrityTests.cs`（238，M7a~M7c + M8）。**修改测试（2）**：`LuceneMaintenanceEngineTests.cs`（805→812，唯一断言 `NotSupportedException` 的用例改判为「索引不存在 ⇒ `ManualRebuildRequired` + 只读不建目录」）· `MaintenanceOptionsBoundaryTests.cs`（352→358，矩阵随新界自动扩域）。**测试 249 → 268（通过 264 / 跳过 4）**；组件与 CLI 均 0 警告/0 错误。`Contracts/*`、`Infrastructure/Supply/*`、任何 `.csproj`、`LuceneSearchEngine.cs`：**0 改动**。
- **三源汇一**：watcher（每 scope 一条 FSW + 一条泵任务）/ mtime checkpoint 补偿扫描 / 低优先级体检，产出**同一份** `FullTextChangeSet` 交执行层。**watcher 批次永不推进 checkpoint**（`requiresCheckpointAdvance: false`）——水位线语义 = 「最近一次成功**补偿扫描**的开始时刻」，让 watcher 推进会吃掉补偿尚未覆盖的窗口。补偿扫描**复算 Delete 候选 = 索引清册 − 磁盘**（且仅当走查 `IsComplete` 时才敢算），并对「索引里没有它」**无条件纳入**（消除「mtime 早于水位线却从未被索引」的永久漏项）。原因合并：`Startup(9) > WatcherOverflow(8) > WatcherError(7) > IndexGenerationChanged(6) > CheckpointUnreadable(5) > ManualRebuildCompleted(4) > QueuePressure(3) > ManualRequest(2) > Interval(1)`（「事件可能已丢 / 状态未知」的原因压过周期性原因，`Majority` 不叠加）。
- **裁定③落地**：同一路径连续 N 轮 retained（`RetainedOldWarnThreshold`，默认 3）⇒ 告警；**明确不得**实现成「自动跳过该文件」——每轮变更集仍含该路径（有断言守住）。checkpoint 读 fail-closed、写为临时文件 + 原子替换。
- **只读体检探针**：三态 `ManualRebuildRequired` / `Healthy` / `Degraded`；索引目录**不存在 / 非索引 / 段损坏** ⇒ `ManualRebuildRequired`，**绝不**通过局部写偷偷初始化、**绝不自动重建**；探针前后索引目录文件数与字节**逐位相同**、且**不得调用执行层**（`BatchCount` 不变）。体检在专用 `BelowNormal` 线程以退避 + `HealthCheckSliceFiles` 切片运行，不可采样（`null`）时**连只读探针都不跑**。
- **默认关闭 ⇒ 零副作用（§1.3 硬不变量 3）的代码级保证**：构造函数只做参数判空 + 指纹计算（零 IO / 零目录推导 / 零线程）；`StartAsync` 首句 `if (!_options.Enabled) return;`；全部路径推导与 checkpoint 读只在 `CreateScopeRuntime`（仅 `StartAsync` 调用）；**唯一** `Directory.CreateDirectory` 在「本轮真的允许推进」的 `WriteCheckpoint` 内；watcher/泵任务只在 `StartAsync` 建、`StopAsync` 收；未运行时 `GetSnapshot().Scopes` 为空 ⇒ 连 `Directory.Exists` 都不发生。`StopAsync` 后自有计数归零（`LiveWatchers=0` / `LivePumpTasks=0` / 体检线程停），进程线程数 25 → 25。
- **变异（父级亲跑复核）**：**M-R1** `UnreadableKind() => Unreadable` 改为 `Missing`（把「读不到」当「不存在」）⇒ 红 **恰好** `M6b_...`（`zzm6boldtext` 命中 1→0，即**旧文档被删**）；**M-R2** 在 `MaintenanceCorpusScan.EnumerateFiles(...)` 前插入 `if (reason == Startup && checkpoint is not null) return false;`（重开时跳过 Startup 全范围校准）⇒ 红 **恰好** `M5_StopModifyRestart_RecoversAndIndexMatchesDisk`（`zzm5beforetext` 命中 0→1，即**停机期修改未恢复**）。两次均 `total=268 / passed=263 / failed=1`、无连带；复原 `bitidentical`、终态 `failed=0 / passed=264`、`MUTATION` 残留 0（活对照 5/11）、`%TEMP%\pudding-fts-s3d-*` 残留 0（**两轮红跑之后仍为 0**）、生产索引根未被触碰。
- ⚠️ **两条「任务书设想 vs 实测」的诚实修正**：① 任务书设想的 ★M-R1 形态「不可读 ⇒ 当 Upsert 投递」**取不到预期红点**——引擎 `LuceneFullTextIndexMaintenanceEngine.cs:481` `var enterDeleteSet = content is not null;` 的提取隔离会保住旧文档，红点只落在「必须产出**空批**」；改用「不可读 ⇒ 当不存在（Delete）」才是维护层真正的危险误用（已有对照实验证据）。② 体检的 `CheckedPathCount` 是**完整**「索引路径集 vs 磁盘路径集」只读比较，被 `HealthCheckSliceFiles` 限制的是**修复面**——即「体检比较面 ≠ 小切片」，如实登记。
- **遗留（未验证 / 待接）**：`DiskHighWatermarkPercent` **目前无生效路径**（默认探针只采 CPU，磁盘维度未采样；旋钮与退避逻辑已就位）；跨 scope 并发与多进程 writer 竞争未构造（本台为单 scope，全局串行只由 S3c `IndexRootWriteGate` 保障）；维护侧新增每变更文件一次宽松共享模式的 `CanOpenForRead` open/close（收益是让「不可读 ⇒ 不产出动作」可达且可测）；`CheckpointAdvancePolicy` 的 fail-closed 饥饿风险被继承（永久不可读/永久提取失败 ⇒ checkpoint 永不推进、每轮重扫不漏文件；M9 告警是人工介入唯一出口）。

## 变更（2026-09-26，S4 边界与安全断言 · 只新增测试，零生产改动）

**目标**：把「`PuddingFullTextIndex` 是叶子级别、无宿主的独立组件」从注释里的**承诺**变成**能失败的机械检查**（`Docs/Conventions/组件化交付规程.md` §3 S4 判据）。

- **新增测试（1 文件 / +13 用例）**：`Source/PuddingFullTextIndexTests/ComponentBoundaryTests.cs`（955 行）。
  - **A1** `ProjectReference` 恰好 1 条（`..\PuddingPathFiltering\PuddingPathFiltering.csproj`，先剥 XML 注释） · **A2** `PackageReference` 名集合恰好 = 冻结的 6 个（多 / 少都红，失败信息分别列「多出的」「缺失的」） · **A3** 无禁用引用目标 + 无向上层反向开放的 `InternalsVisibleTo`（开放面冻结 = `PuddingFullTextIndexTests` / `PuddingMemoryEngine` / `PuddingMemoryEngineTests`） · **A4** 测试工程 `ProjectReference` 恰好 = 组件 · **A5**（2 条）测试进程未加载禁用程序集：force-load 元数据引用 + `*.deps.json` 声明闭包**交集** + 真空防护；声明闭包自身也 0 命中（确定性引信） · **A6** 组件源码 0 命中调用方程序集名（**只剥注释、不剥字符串字面量** —— 否则 §4/M1 的字符串变异无法取红） · **A7** 组件内不复刻 `PathNoiseRules` **类型定义**（真源在 `PuddingPathFiltering`，允许 `using` 使用） · **A8** SHA-256 类型名只出现在 **4** 个真源文件 · **A9a** `IFullTextSearchEngine` 公开成员冻结（`BuildIndexAsync(3)` / `HasIndex(1)` / `RemoveIndex(1)` / `SearchAsync(7)`，反射取运行时真值） · **A9b** `.last_indexed` stamp 恰为 `{t,p}`（源码侧写 / 读方法体 + 运行时 JSON 形状；`t` 必须 UTC ISO `…Z`） · **A10** maintenance 不得走 `BuildIndexAsync` / `RemoveIndex` / 供给协调器类型名 / `OpenMode.CREATE`（`\bCREATE\b` 词边界 + 大小写敏感 ⇒ 不误伤 `CREATE_OR_APPEND` 与 `IndexSizeReport.Create(`）。
  - 每条断言都带**探测器自检**或**真空防护 / 正对照**（喂样本给探测器、扫描面非空、注释剥离自检、组件侧正对照、`CREATE_OR_APPEND` 防假红自检）。
- **测试 268 → 281（通过 277 / 跳过 4 / 失败 0）**；组件构建 **0 警告 / 0 错误**；CLI（`PuddingFullTextIndex.Cli`）构建 **0 警告 / 0 错误**、CLI 测试 **45/45**。
- **变异取红（两条，父级亲跑）**：**M1** 组件 csproj 临时加 `..\PuddingCodeIndex\PuddingCodeIndex.csproj` 引用（**build 仍成功**）⇒ **共 5 条红**：`A1_Component_Project_References_Only_The_Path_Filtering_Leaf` / `A3_Component_Project_Has_No_Forbidden_References` / `A5_Test_Process_Must_Not_Load_Forbidden_Assemblies` / `A5_Test_Dependency_Closure_Must_Not_Contain_Forbidden_Assemblies` / `S1b_Boundary_Component_Csproj_References_Only_Allowed_Projects`（**父级亲跑复核实测；子代理自述少报 1 条**），逐条点名 `..\PuddingCodeIndex\PuddingCodeIndex.csproj` / `PuddingCodeIndex`，`M1_TRX total=281 passed=272 failed=5` 且 `M1_BUILD_EXIT=0`（**不是靠编译失败冒充红**）；复原 `git hash-object` = `f276a583…` **逐位相同**，复跑 `GREEN_TRX total=281 passed=277 failed=0`。**M2** 在 `Infrastructure/` 临时新增第二份 `SHA256.HashData` 命名实现 ⇒ **恰好 A8 红**并点名 `Infrastructure\BoundaryMutationSha256Probe.cs`；删净后 57 个组件 `.cs` 的 SHA-256 与变异前快照 **diff = 0**、`BoundaryMutationSha256Probe` 在源码与重编译 DLL 中均 0 命中，复跑全绿。
- **零生产改动**：终态 `git status --porcelain -- Source/PuddingFullTextIndex Source/PuddingFullTextIndexTests` 仅 `?? Source/PuddingFullTextIndexTests/ComponentBoundaryTests.cs`（本条目是唯一文档追加）。
- **实测与任务书不符（登记，逐条见 `temp/s4-report.md`）**：① A8 的 SHA-256 真源实测 **4 个文件**（任务书写 2 个；`Supply/FileSupplyLease.cs` 与 `Supply/SupplyIndexDirectoryLayout.cs` 各持一份**供给层命名用** SHA-256）；② 任务书 §5 的 CLI 工程名 `PuddingCodeIndexer.Cli*` **不存在**，真名是 `PuddingFullTextIndex.Cli` / `PuddingFullTextIndex.Cli.Tests`；③ A6 剥注释但**不剥字符串**（否则 M1 备选变异形态取不到红）；④ maintenance 内有 1 处**注释**提及 `StagedFullTextIndexBuilder`（`Infrastructure/Maintenance/ScopeReaderInvalidation.cs:21`），剥注释后 A10 = 0 命中。

---

## 变更（2026-09-26，S3e：跨路径互斥的可证性 · **测试-only、零生产改动**）

- **新增测试（1 文件 / +20 用例）**：`Source/PuddingFullTextIndexTests/CrossPathMutexTests.cs`（672 行，M1 / M2×13 / M2b / M3 / M4a / M4b / M4c / M5）。**测试 281 → 301（通过 297 / 跳过 4 / 失败 0）**；组件构建 **0 警告 / 0 错误**；CLI（`PuddingFullTextIndex.Cli`）构建 **0 警告 / 0 错误**、CLI 测试 **45/45**。
- **§2 实测出的承载面（与任务书设想不同，已按实测调整断言对象）**：`FullTextChangeSet.ScopeKey` 是**不透明字符串**（`Contracts/FullTextChangeSet.cs:78` 只声明「与既有供给链同口径」，记录本身**不做**规范化）；维护循环**逐字透传**调用方给的 scope 描述符（`LuceneFullTextIndexMaintenance.cs:1021` 用 `scope.ScopeKey` ← `FullTextMaintenanceScope`，契约原文「调用方负责把配置里的原始 scope 路径解析成 RootPath / ScopeKey」，`Contracts/IFullTextIndexMaintenance.cs:62`）。**组件内不存在「维护 scope 描述符工厂」**，可见的维护侧键推导真源是 `FullTextChangeCoalescer.NormalizeComparisonKey`（被 `MaintenanceOptions.cs:416` 的 scope 解析、`LuceneFullTextIndexMaintenance.cs:1539` 的 `FindScopeLocked`、`LuceneFullTextIndexMaintenanceEngine.cs:917` 的 `ProbeIntegrityAsync` 使用）⇒ **M1/M2/M4 的维护侧入口取它**；供给侧入口 = `SupplyScopeNormalizer.Normalize`（`:92` → `ToScopeKey` `:121`）。
- **I-MUTEX 判定（三态，均为行为/实测证据）**：① **普通目录 = 已证实** —— 两条路径都落到 `SupplyScopeNormalizer.ToScopeKey`，键**逐字符相同**且租约文件同一（实测 `c:\users\huany\appdata\local\temp\pudding-fts-s3e-*\corpus`；M4b 行为级：供给侧持租约时维护内核按自己的键取租约失败 ⇒ `FullTextMutationState.Busy`、`CheckpointAdvanced=false`、`CommitMilliseconds=null`、reader 失效 0 次、消息「互斥失败：跨进程租约在 200 ms 的有界等待内未取得…本次不接管」；释放后同一变更集 `Applied`）。② **盘根形态 = 已证伪** —— 供给侧 `TrimTrailingSeparators` **保住盘根**（`C:\` ⇒ `c:\`，`SupplyScopeNormalizer.cs:127-134`），而 `NormalizeComparisonKey` 先 `TrimEnd` 且**只在裁剪后为空时**才补回（`FullTextChangeCoalescer.cs:283-292`）⇒ `C:\` 得 `c:`；`C:\` 实测：supply=`c:\` / maintenance=`c:` ⇒ **不同租约文件 ⇒ 同一语料根上两条路径不互斥**（`C:\` 是被 `SupplyScopeNormalizer` 接受、且不被 `MaintenanceOptions` 拒绝的合法 scope）。③ **同进程默认身份 = 不互斥** —— `FileSupplyLease` 按 `OwnerId` 判「自己人」重入（`:210`/`:236`），而供给协调器（`FullTextIndexSupplyCoordinator.cs:50` ← `SupplyCoordinatorOptions.OwnerId` 默认 `SupplyLeaseOwner.ForCurrentProcess()`）与维护内核（`LuceneFullTextIndexMaintenanceEngine.cs:297` 硬编码 `ForCurrentProcess()`）默认同主 ⇒ 实测第二条路径**重入成功**并覆盖租约文件的 `JobId`（`maintenance-batch-01` → `supply-job-01`）；两条路径的**进程内**闸门也不是同一把（协调器 `_scopeGates` vs `LuceneSearchEngine.GetScopeGate` + `IndexRootWriteGate`）。⇒ 跨进程互斥成立的条件是「owner 可区分」。
- **变异取红（父级亲跑，各三份证据）**：**M-X1**（`FullTextChangeCoalescer.cs` 维护侧键临时 `+ "x"`）⇒ `s3e-mx1-red.trx` **failed 16 / passed 4 / total 20**，红点 = M1、M2（13 条中 11 条）、M2b、M4a、M4b、M4c（绿的那 4 条 = M3、M5 与 M2 的两条「合法不对称」输入）；复原 `git hash-object` = `1efe483ae9287baa32f7ca4c5c1b986d72928131` **逐位相同**，复跑全绿 `total=301 passed=297 failed=0`。**M-X2**（`SupplyScopeNormalizer.ToScopeKey` 临时恒等）⇒ `s3e-mx2-red.trx` **failed 3 / passed 17**，红点 = `M2b`（大小写折叠 + 「键不得原样回传入参」）、`M2('drive-root-invariant-falsified')`、`M3`（防空转正对照）；复原 `git hash-object` = `3de93c5b1ca0afe3f54ce390162cd0b0466c595f` **逐位相同**，复跑全绿 `total=301 passed=297 failed=0`。 **父级独立复现（全量套件口径，非仅本片文件）**：M-X1 ⇒ `total=301 passed=249 failed=48` —— **48 = 本片 16 + 32 条既有用例**（`NormalizeComparisonKey` 是合并/去重/gate 的公共口径，故 `+x` 波及既有切片），本片 16 条红点与上表**逐条一致**（11 条 M2 行 + M1/M2b/M4a/M4b/M4c）；M-X2 ⇒ `total=301 passed=287 failed=10` —— **10 = 本片 3 + 7 条既有用例**。⇒ 上表的 16/3 是**限定本片文件**的口径，两者不矛盾。父级另取**二进制级**复原证据：`PuddingFullTextIndex.dll` SHA-256 基线 `046F2631572169B4CFEC3E31665A1B0F23D9D5901F5C165223029024D864C9E1` → 变异期 `0D7E1C0C6CFDD34BB52A809CEA5FBE8AB906748A8A79F81544633E01BD522729` → `-t:Rebuild` 后**逐位回同** `046F2631…`。`S3E-MUTATION` 残留 0。
- **守恒（R7）**：把本片测试文件**临时移出** ⇒ `s3e-conservation-no-newfile.trx` **total=281 / passed=277 / skipped=4**（= 基线；与当时 626 行版本 `git hash-object = 0ccdedf11b5712b3d4863e13d36235e92d0884df` 相比 ⇒ 该版本 +19）。移回后该版本 hash **逐位相同**；随后补入 `M2b`（+1，**M-X2 的取红靶点**）⇒ 终态 **672 行 / 301 用例 / hash `1c8ae1076da096834e2aa82c4b8cc5b0f54a6b3e`**；不含本片文件时仍为 281。
