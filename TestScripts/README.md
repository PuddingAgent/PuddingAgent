...
`Runtime` 1658 / 0, `Platform` 1363 / 0, `AdminJest` 1347 / 5 known-red
(3 red suites; eleven cases were fixed on 2026-09-21 - one **real defect** (missing admin menu icon
mappings for `hdd`/`key`), one flaky suite calibrated (`jest.setTimeout`), one time-bomb fixture
(relative dates), one copy-migration case, and seven other test-lag cases; see `known-red-dispositions.md`).
