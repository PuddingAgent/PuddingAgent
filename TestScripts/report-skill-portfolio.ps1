...
function New-ContentAuditReport {
    <#
      ...
      回答「技能有没有用」里**能靠静态内容回答**的那一半：
        - 是不是单次会话的自动回放（来源段 + 置信度）；
        - 有没有**边界知识**（前提/约束/不适用/陷阱）—— 模型最缺、也最容易被过时技能误导的部分；
        - 正文有多少行是「调哪几个工具」的调用序列（模型本来就会的那部分）。
      不能回答的一半：是否真的被命中、命中后是否让任务更好 —— 那需要使用遥测（当前尙不存在）。
    #>
    param([object[]]$Entries, [string]$Root)

    $boundaryPatterns = @(
        '## 约束', '## 边界', '## 前提', '## 不适用', '## 陷阱', '## 注意事项', '## 注意',
        '## 失败', '## 回滚', '## 风险', '## 限制', '## 反模式',
        '## Precondition', '## Constraint', '## When NOT', '## Anti-pattern'
    )
    $stepToolPattern = '^\s*\d+\.\s+`[a-z_][a-z0-9_]*`'
    ...
    $toolStepTotal = 0
    $lineTotal = 0
    $confidences = New-Object System.Collections.Generic.List[object]
    $sizes = New-Object System.Collections.Generic.List[object]
    $missingBoundary = New-Object System.Collections.Generic.List[string]
    $versions = @{}

    foreach ($entry in $enabled) {
        $skillMd = Join-Path (Join-Path $Root $entry.SkillId) 'SKILL.md'
        if (-not (Test-Path -LiteralPath $skillMd)) { continue }
        $text = [string](Get-Content -LiteralPath $skillMd -Raw -Encoding UTF8)
        if ($null -eq $text) { $text = '' }

        $sizes.Add([int]$entry.BodyBytes)
        $tags = @($entry.Tags)
        if ($tags -contains 'auto-generated') { $autoGenerated++ }
        if ($text.Contains('## 来源')) { $withSource++ }

        $confMatch = [regex]::Match($text, '置信度[:：]\s*(\d+)\s*%')
        if ($confMatch.Success) { $confidences.Add([int]$confMatch.Groups[1].Value) }

        $hasBoundary = $false
        foreach ($pattern in $boundaryPatterns) {
            if ($text.Contains($pattern)) { $hasBoundary = $true }
        }
        if ($hasBoundary) { $withBoundary++ } else { $missingBoundary.Add($entry.SkillId) }
        if ((-not $hasBoundary) -and $text.Contains('## 来源')) { $replayOnly++ }

        foreach ($bodyLine in ($text -split "`n")) {
            $lineTotal++
            if ($bodyLine -match $stepToolPattern) { $toolStepTotal++ }
        }

        $version = $entry.Version
        if ($version) {
            if (-not $versions.ContainsKey($version)) { $versions[$version] = 0 }
            $versions[$version]++
        }
    }
    ...
    $medianSize = 0
    $avgSize = 0
    if ($sortedSizes.Count -gt 0) {
        $minSize = $sortedSizes[0]
        $maxSize = $sortedSizes[$sortedSizes.Count - 1]
        $medianSize = $sortedSizes[[int][Math]::Floor($sortedSizes.Count / 2)]
        $sum = 0
        foreach ($size in $sortedSizes) { $sum += $size }
        $avgSize = [Math]::Round($sum / $sortedSizes.Count)
    }
    $avgConfidence = 0
    if ($confidences.Count -gt 0) {
        $sum = 0
        foreach ($value in $confidences) { $sum += $value }
        $avgConfidence = [Math]::Round($sum / $confidences.Count, 1)
    }
    $toolStepPct = 0
    if ($lineTotal -gt 0) { $toolStepPct = [Math]::Round(100.0 * $toolStepTotal / $lineTotal, 1) }

    $lines.Add('# 技能内容审计（只读）')
    $lines.Add('')
    $lines.Add("生成时间：$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss zzz')")
    $lines.Add("技能根目录：$Root")
    $lines.Add('')
    $lines.Add('> 只读产出。本报告回答「技能有没有用」里**能靠静态内容回答**的那一半。')
    $lines.Add('> **不能**回答的一半：技能是否真的被命中、命中后是否让任务更好 —— 那必须有使用遥测（当前不存在）。')
    $lines.Add('')

    $lines.Add('## 1. 这些技能是怎么来的')
    $lines.Add('')
    $lines.Add('| 指标 | 值 | 占总启用技能 |')
    $lines.Add('|------|----|--------------|')
    $total = $enabled.Count
    $lines.Add("| 启用技能总数 | $total | 100% |")
    $pctAuto = 0
    if ($total -gt 0) { $pctAuto = [Math]::Round(100.0 * $autoGenerated / $total, 1) }
    $lines.Add("| 标记 auto-generated | $autoGenerated | $pctAuto% |")
    $pctSource = 0
    if ($total -gt 0) { $pctSource = [Math]::Round(100.0 * $withSource / $total, 1) }
    $lines.Add("| 正文含 `## 来源` 段（会话回放式） | $withSource | $pctSource% |")
    $lines.Add("| 可提取到置信度 | $($confidences.Count) | 平均 $avgConfidence% |")
    $lines.Add('')

    $lines.Add('## 2. 有没有边界知识（最关键）')
    $lines.Add('')
    $lines.Add('边界知识 = 前提 / 约束 / 不适用场景 / 陷阱 / 回滚。这些是**模型最缺、也最容易被过时技能误导**的部分：')
    $lines.Add('一条只讲「怎么做」的技能，对强模型是冗余；一条讲错「什么时候不该做」的技能，是**负价值**。')
    $lines.Add('')
    $pctBoundary = 0
    if ($total -gt 0) { $pctBoundary = [Math]::Round(100.0 * $withBoundary / $total, 1) }
    $pctReplay = 0
    if ($total -gt 0) { $pctReplay = [Math]::Round(100.0 * $replayOnly / $total, 1) }
    $lines.Add('| 指标 | 值 |')
    $lines.Add('|------|----|')
    $lines.Add("| 提到边界知识的技能 | $withBoundary（$pctBoundary%） |")
    $lines.Add("| **纯回放**（有来源段 + 零边界知识） | **$replayOnly（$pctReplay%）** |")
    $lines.Add('')
    $lines.Add('缺边界知识的技能（最多列 20 个）：')
    $lines.Add('')
    foreach ($skillId in ($missingBoundary | Select-Object -First 20)) { $lines.Add("- $skillId") }
    $lines.Add('')

    $lines.Add('## 3. 正文有多少是「调哪几个工具」')
    $lines.Add('')
    $lines.Add('形如 ``N. `tool_name``` 的行 = 工具调用序列。这部分是**模型本来就会的**（工具描述已包含用法）：')
    $lines.Add('')
    $lines.Add('| 指标 | 值 |')
    $lines.Add('|------|----|')
    $lines.Add("| 工具调用行数 | $toolStepTotal |")
    $lines.Add("| 正文总行数 | $lineTotal |")
    $lines.Add("| 占比 | $toolStepPct% |")
    $lines.Add('')
    ...
    $lines.Add('| 指标 | 字节 |')
    $lines.Add('|------|------|')
    $lines.Add("| 最小 | $minSize |")
    $lines.Add("| 中位 | $medianSize |")
    $lines.Add("| 平均 | $avgSize |")
    $lines.Add("| 最大 | $maxSize |")
    $lines.Add('')

    $lines.Add('## 5. 版本分布（用户提到的「仅适用某版本」在数据里的影子）')
    $lines.Add('')
    $lines.Add('| 版本 | 技能数 |')
    $lines.Add('|------|--------|')
    foreach ($version in ($versions.Keys | Sort-Object)) {
        $lines.Add("| $version | $($versions[$version]) |")
    }
    $lines.Add('')

    $lines.Add('## 6. 这张表能证明什么、不能证明什么')
    $lines.Add('')
    $lines.Add('**能证明**：这批技能的来源与形态高度同质（自动提炼、会话回放式、以步骤为主）；')
    $lines.Add('对「模型已经会做」的部分做了大量抄录，而**边界知识（何时不该用）覆盖很低**。')
    $lines.Add('')
    $lines.Add('**不能证明**：它们**没有用**。没有使用遥测，就无法区分三种情况：')
    $lines.Add('① 确实被命中且帮上了（应保留）；② 从未被命中（应淘汰或重建触发面）；')
    $lines.Add('③ 被命中但把模型带偏（**负价值**，最危险）。本报告无法区分 ①②③。')
    $lines.Add('')
    $lines.Add('⇒ 这正是「索引 + 调用次数 + 成功率」遥测基础设施要解决的问题。')
    $lines.Add('')

    Write-Host ("CONTENT enabled={0} autoGenerated={1} withSource={2} withBoundary={3} replayOnly={4} toolStepPct={5}" -f `
            $total, $autoGenerated, $withSource, $withBoundary, $replayOnly, $toolStepPct)
